  ...
  if (pathname === '/api/capabilities' && method === 'GET') {
    sendJson(response, 200, {
      jobs: [
        ...
        { jobType: 'span-rebuild', executionMode: 'deterministic', supported: true, note: 'deterministically slices assets/reports, then uses the local qwen text LM to review usable speech text and generate Chinese materialPatterns from minimal span facts' },
