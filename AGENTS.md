...
- Treat workspace `剪辑规则` as the formal edit-flow definition after Chronology:
  ...
  - Resolve `[Edit]` media relink maps every configured `project-brief` root path candidate to the currently readable local root, calls Resolve `RelinkClips`, saves the existing project, and reports media-pool/timeline unreadable, unmapped, and old-path counts; it must not export a DRP unless the user separately triggers a DRP action
