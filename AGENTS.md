...
- `timeline.generate` must automatically mark Resolve rough-cut clip colors for downstream manual batching: ordinary audible video items and their linked audio items use `Orange`; photo video items use `Blue`; timelapse video items use `Purple`; when a timelapse clip is audible, keep the video item `Purple` and mark only linked audio items `Orange`. The host must validate `SetClipColor()/GetClipColor()` results and block on failure.
