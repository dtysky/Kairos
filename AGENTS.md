...
- The post span-builder speech-window review is also the formal transcript-correction stage. Raw ASR in `analysis/asset-reports/*.json`, segment boundaries, and segmentation are immutable; reviewed text is written only to speech/mixed spans and audited under `analysis/transcript-reviews/<inputsHash>.json`. Workspace `config/transcript-glossary.json` entries contain only a unique canonical spelling, optional pronunciation, and required usage context. Pronunciation only recalls a possible homophone; the Agent must match the full sentence, neighboring speech, and fresh time-scoped Pharos/manual-itinerary context before using the canonical spelling. A proper name unsupported by glossary context or current trip context must enter a `transcript-correction` Review at stage `chronology`. Any unresolved correction keeps `spans.meta.status=pending-speech-review`; only an atomically validated final review may write `fresh`. Visual spans must never inherit or be reshaped by transcript correction.
...
- Workspace ASR routing is explicit in `config/runtime.json.asr.backend`, whose only user choices are `qwen3` and `whisper`; never silently fall back between them. Qwen3 auto-discovers the convention-based `models/` checkpoint pair for Apple Silicon/MLX or Windows + NVIDIA/Transformers-CUDA and formally requires both Qwen3-ASR and Qwen3 ForcedAligner. Model paths, language routing, and audio chunk sizing are implementation details, not `/project` fields. Never fall back across platform checkpoint formats. Do not inject `config/transcript-glossary.json` or itinerary terms into Qwen context or Whisper hotwords/prompts; those sources belong only to post-ASR Agent review. ML health, the Supervisor Analyze monitor, and `/analyze` must expose configured/actual backend, runtime variant, device, model, aligner, availability, and blocker; `/project` only selects the backend. An unavailable or mismatched backend must block Analyze.
