...
- `materialPatterns[]` remains `string[]`; the LM prompt requests exactly seven tags: four deterministic leading tags, one LLM-authored `情景故事` slot, then two LLM-authored free tags. The first four are controlled `视角类型`, extractive `当前环境`, natural observable `天气光线`, and binary `口播语音` (`有口播语音` / `无口播语音`). Entry 5 should be a short scene-story phrase or `情景不明`; entries 5-7 come from the LM on normal rows and code must not invent free tags. Missing slot 5 enters the failed-span list, is retried as a single span after the main chunks, and only then falls back to fixed `情景不明` so the long run can finish.
- `/chronology` must surface active `span-rebuild` progress from `.tmp/chronology/progress.json`, including 10-span text-LM chunk progress, failed-list retry progress, retry/repair count, warning state, failed/recovered/fallback counts, and the `.tmp/chronology/span-rebuild.partial.json` checkpoint for completed materialPatterns and failed spans.
...
- `chronology-build` treats Pharos point events as hard route boundaries. Pharos `continuous` only provides route time context / route summary context; its route prose such as `深圳 → 南宁 全程` must never be written to chronology `event.location`, `route.from`, or `route.to`. `drive` / route-like in-car spans, plus `aerial` drone-follow spans with clear follow-car / moving-vehicle evidence, must remain in the event-to-event `route` even when short-span GPS looks stationary, and in-car speech/subtitles must summarize inside the route rather than split new events.
