...
- Treat workspace `剪辑规则` as the formal edit-flow definition after Chronology:
  ...
  - `resolve.media_sync` is the deterministic Resolve Media Pool sync step; it organizes the project-global `Kairos Project Media` bin by chronology event title, reuses existing MediaPoolItems, and does not write a `media-archive.json`
  - `timeline.generate` must consume Flow Plan declared predecessor outputs, select already-synced Resolve Media Pool items, validate source ranges after native append, and must not require `edits/<editId>/script/current.json`
