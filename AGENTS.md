...
- `timeline.generate` must add default Resolve source handles to selected audible speech/mixed non-photo clips before append: `240ms` head and `720ms` tail, clamped to asset duration. The generated source-speech SRT remains transcript-timed inside those wider clip bounds and must not stretch cues across the handle padding.
- Post-lock narration framework facts for no-subtitle visual/aerial/timelapse entries must come from `visualObservation` only, never `materialPatterns`. If the current no-subtitle Resolve clip points at `semanticKind=speech/mixed`, first map its actual source range to a same-asset visual span (`overlap` before nearest `<=15s`), then use that visual span's `visualObservation`; only when no visual span qualifies may the generator fall back to the current speech/mixed span's own `visualObservation`.
