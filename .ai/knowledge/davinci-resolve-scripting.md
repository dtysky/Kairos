...
- For Resolve rough cuts, Kairos uses `TimelineItem.SetClipColor()` / `GetClipColor()` as user-facing batch markers in the Edit/Fairlight Timeline Index: ordinary audible video items and their linked audio item(s) use `Orange`, photo video items use `Blue`, and timelapse video items use `Purple`. If a timelapse clip is audible, keep the video item `Purple` and mark only its linked audio item(s) `Orange`. Rough-cut generation also creates or reuses Color Groups named `Kairos Photos` and `Kairos Timelapse`, assigning photo and timelapse video items respectively so Color-page `Group Post-Clip` effects can be applied in batches. Do not use Color-page filters or flags as replacements for clip colors: Color-page selection does not become Edit-page timeline selection, and Resolve flag behavior propagates across linked items imprecisely.
