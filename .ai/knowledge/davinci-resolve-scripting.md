...
- For generated voiceover audio, create or reuse Media Pool bin `Kairos Voiceover / <timelineName>`, set it as the current folder, import the audio file with `MediaPool.ImportMedia`, restore the previous folder when possible, and append it with `MediaPool.AppendToTimeline([{"mediaPoolItem": item, "mediaType": 2, "trackIndex": audioTrackIndex, "recordFrame": frame}])`. The documented `Project.InsertAudioToCurrentTrackAtPlayhead(...)` depends on Fairlight selected track/playhead state and is less deterministic for batch subtitle insertion.
