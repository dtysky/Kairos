...
- When multiple subtitle rows are selected in the Volc voiceover plugin, the selection itself is the grouping contract: sort selected rows by timeline time, synthesize one merged audio file, and append it at the first selected subtitle's start frame.
- The Volc voiceover plugin reads `volcApiKey` and registered voice profiles from workspace global `config/runtime.json.voiceover`. The Resolve panel should expose a compact profile selector rather than raw API key / speaker / resource / model / language fields. Generated audio artifacts should resolve through the matched Kairos project's `config/project-brief.json.voiceoverMedia` primary / alternate path candidates, optionally matching explicit `voiceoverMedia.resolveProjectAliases[]`, and write formal outputs to `<voiceoverMediaRoot>/<safe Resolve project>/<safe timeline>/` with only Resolve-importable `vo_<unit>_<request>.mp3` files in that media directory. `voiceoverMedia` is project configuration surfaced in `/ingest-gps`, and `/edit` Resolve relink should include the `Kairos Voiceover` Media Pool bin when it is configured. Request cache, bridge job files, backend stdout, debug manifests, and plugin logs belong under the matched project's `.tmp/resolve-volc-voiceover-plugin/`.
...
- The Volc voiceover Lua menu script must remain platform-aware. On Windows it cannot call macOS paths such as `/usr/bin/python3`, `/bin/mkdir`, or `/usr/bin/open`; it should resolve Python from `KairosVolcVoiceoverLib/kairos_workspace.json` `pythonExecutable` or `KAIROS_PYTHON`, quote paths for the host shell, use Windows-compatible directory/open commands, and force UTF-8 output for Lua-to-Python command capture so Chinese profile names render correctly in Fusion UI controls. On macOS it may keep `/usr/bin/python3` as the default backend executable.
