...
- For selected audible speech/mixed non-photo clips, `timeline.generate` must first prefer `span.effectiveSpeechStartMs/effectiveSpeechEndMs` as the effective spoken source window when both are valid, then expand that window by default handles (`300ms` head, `300ms` tail, clamped to asset duration) before append. When effective speech boundaries are missing or invalid, fall back to the span source/edit window. Aerial clips are forced muted first, even when ASR produced transcript or `speech/mixed` truth, so they do not receive source-speech handles. The companion source-speech SRT still maps transcriptSegments to their true times inside the wider clip; do not stretch subtitle cues across the handle padding.
- When migrating an existing SRT into a Resolve rough cut, pass it to the host as a preloaded `subtitleSrtPath` before video append. Do not append an SRT after a populated video timeline and assume `recordFrame` will place it; Resolve appends SRT media at the existing timeline end. The host must import a unique content-hashed copy of the SRT rather than reusing the same path, because Resolve can reuse stale Media Pool subtitle media for an unchanged file path.
