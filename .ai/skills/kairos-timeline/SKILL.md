...
- When migrating an existing SRT into a Resolve rough cut, pass it to the host as a preloaded `subtitleSrtPath` before video append. Do not append an SRT after a populated video timeline and assume `recordFrame` will place it; Resolve appends SRT media at the existing timeline end.
