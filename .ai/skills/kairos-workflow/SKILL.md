...
- `DaVinci color` 当前已有独立 `/color` 主路由，但当前阶段先承载 root 级配置与状态，不等于 Resolve 执行链已经全量接通
- `/color` 当前应自动发现已配置 `rawPath` 的素材根，并派生 root 级默认命名与阻塞信息；不要把“还没接通 Resolve”误渲染成“没有可显示 root”
