...
- Windows Resolve 21 + MP4/H.265 固定码率必须走 host-owned generated render preset XML：每次从 Kairos root `renderPreset` 生成干净 XML，再 `ImportRenderPreset / LoadRenderPreset`；不得从当前 Deliver 页 `SaveAsNewRenderPreset` 继承 UI 状态，也不得用它当导入后校验来源。导入后必须直接 `ExportRenderPreset(presetName)` 校验 named preset，因为 live probe 证明当前 Deliver UI 可粘滞显示 `AlternateInFolder=1`。生成 XML 必须写入并验证 `RecordFormatSubType=hvc1_qsv`、`h264_datarate = root.color.renderPreset.bitrateKbps`、`encoder_command_param_map.rc=CBR`、`encoder_command_param_map.bitrate=<bitrateKbps>`；不要在 Windows H.265 上用会被拒绝的公开 `VideoQuality` key 作为正式路径。
