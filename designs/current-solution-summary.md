...
- `timeline.generate` 当前默认按 span 的自然 source 时长 / edit-friendly bounds 摆放 clip，`speed` 来自 `material-slots.json.treatments` 覆盖值，缺省为 `1`；读取时会把 raw speed 规整为 `1..5` 的整数倍。
