...
## 2026-09-05 ASR 字级真值与确定性字幕拆句

- Analyze 的音频链路是 `ASR + ForcedAligner -> aligned-token timing repair -> transcript-segmentation -> finalize`。ASR 只产出 `rawText + alignedTokens[]`；逐字 token 保存稳定索引、毫秒起止和到下一 token 的 `gapAfterMs`。程序先验证字符覆盖、索引、单调时间与 gap，再按单素材的 token 时长分布识别吞入长静音的离群 token，收缩异常区间并记录原始/修复时间；字符、顺序和 `rawText` 不得改变。整条没有任何正时长 token 时，rawText 仅用于审计，不生成字幕或推测时间。
- 字幕拆句不再调用 Qwen3.5。程序按 ASR 原始标点确定性生成句界：只有中文 `，` 与英文 `,` 是软标点，其余 Unicode 标点都结束当前句；连续标点合并为同一个边界。超过 36 个非标点字符的句子只允许在已有逗号处均衡拆开，并把被选中的逗号改为句号；相邻句的 token 间隔严格小于 1500ms 且合并后不超过 36 字时再顺序合并，内部标点原样保留。最终 segment 正文完整覆盖原 token，时间只由首尾 token 派生。
- `analysis/audio-checkpoints` 升级到 schema v3，并固定 `alignmentContractVersion=qwen3-character-alignment-v2`；ForcedAligner 返回的中文、英文和数字 token 都按 Unicode 字符展开。旧 v2 schema、v1 对齐契约以及缺少当前契约标记的开发期 v3 checkpoint 不再直接复用。asset report 保存 `asrRawText / alignedTokens / transcriptSegmentation` 审计字段，正式 `transcriptSegments` 使用当前确定性拆句结果。
- ASR transcript 不再生成固定 `confidence=0.8` 的 `evidence`。OCR、术语、空间与其它业务 evidence 不受影响。后续 speech-window review 只能校正 segment 文本，不能重新拆句、合句或修改时间。
- Analyze `speechWindows` 是独立于字幕拆句的确定性时间结构：它在字级时间修复后读取有效 `alignedTokens`，以单素材 log-gap 上尾、token 时长节奏保护、单候选显著性和稀疏自然断层决定边界。规则不读取字幕句界、文字、标点或字数，不使用固定秒数和最长窗口；恢复已有 v3 audio checkpoint 时从保存的 `alignedTokens` 重算窗口而不重跑 ASR；结果仍参与统一视觉决策、fine-scan 和素材召回。
- Windows 将 Qwen ASR 放在独立 `.venv-asr` worker，主 `.venv-ml` 保持 Qwen3.5 所需 Transformers 版本；worker 优先使用 loopback 端口 8911，若真实 bind 探测发现端口被占用则由 Supervisor 分配可用临时端口，并把实际 URL 注入主 gateway。Supervisor 统一启停两者，ASR 结束后先卸载模型并终止本轮记录的 worker，再进入文本/VLM 阶段，不抢占或终止外部占用进程。
- Analyze 监控必须区分 ASR `ready / released / unavailable / not-running` 生命周期。本轮进入 `transcript-segmentation` 或更晚阶段后，独立 worker 端口关闭属于 `released` 成功态，页面显示“ASR 已完成并释放显存”，不得把连接拒绝渲染成配置阻塞；只有 ASR 尚需运行时的真实缺失或 backend 不一致才是 blocker。
...
## 1.3 2026-09-06 口播与字幕统一审查

`span-rebuild` 后的正式人工闸门统一为“口播与字幕审查”。程序先应用确定性简体中文正字归一和工作区精确文字归一，Agent 再对归一后文本生成可审查建议；其余字幕文本和 speech-window 变更在用户提交整轮审查前不得写入正式 spans。审查事实写入 `analysis/speech-transcript-reviews/<inputsHash>.json`，同源 Markdown 报告写入项目 `.tmp/chronology/`，Console `/chronology` 直接读取 JSON，不解析 Markdown。

- 报告只输出有动作或字幕有疑点的记录，并按类别拆成五张独立表：`字幕｜已自动修正`、`字幕｜建议修正`、`字幕｜需人工听音`、`口播窗口｜建议裁切`、`口播窗口｜建议取消`。保持原文、保持原窗口和无需调整的现场声音不进入报告。`口播窗口｜需人工听音` 已取消：Agent 无法可靠提出连续裁切或取消时直接保持原窗口，不产生人工任务。
- `建议修正 / 建议裁切 / 建议取消` 在 Console 中默认接受，用户只需手动改为不接受；默认值只是审查草稿状态，点击提交前不写正式 spans。只有 `字幕｜需人工听音` 默认未决并阻止整轮提交，状态列只提供单个 `审查完成` Radio，最终文本直接取可编辑文本框。完成状态只表示用户已经听音并确认当前文本，不再用 `接受修改 / 保留原文` 同时表达状态和结果。旧草稿中的字幕 `rejected` 采用原文、`accepted` 保留已编辑文本；旧 `speech-window-needs-listening` 记录按保持原窗口迁出审查产物，不改变 spans。
- Console 中的勾选、文本、听音结论和裁切范围在修改后自动保存到 review artifact，并明确显示 `草稿保存中 / 草稿已保存 / 草稿保存失败`；刷新页面必须恢复服务端草稿。草稿保存不应用 spans，最终提交仍是唯一正式写入口。
- `字幕｜需人工听音` 行提供素材原音的区间循环播放。页面请求审查区间前后各 `1000ms` 的上下文并在素材边界截断；这段余量只服务试听，不改变字幕、speech window 或审查建议的正式时间。Supervisor 只按 `projectId + assetId + startMs/endMs` 从受信任的资产索引解析素材并生成浏览器可播放的缓存音频；页面不接收任意本地路径。切换播放行时停止上一段，用户可反复听同一范围。
- `建议裁切` 只能使用已有 transcript segment 的边界形成一个或多个保留范围；`建议取消` 只取消该 speech window 的口播属性和字幕，必须保留同范围已有的 visual span 参与视觉召回。若没有可验证的 `visualObservation`，不得建议取消，只能进入人工听音。
- 提交时服务端重新校验 `inputsHash`、原文 hash、segment 时间、窗口范围、视觉保留、effective glossary、文字归一和行程 hash，再原子写入 spans、review artifact 与 meta；任何未决项或校验错误都保持 `pending-speech-review`，不得部分完成。
- effective glossary 由只读内置 `resources/transcript-glossaries/*.json` 和工作区 `config/transcript-glossary.json` 合并。内置表收录丁达尔现象等稳定摄影/视频术语；工作区表保留人名、地名、菜名、型号和个人用语，同名条目可覆盖内置语境。两者均为 `schemaVersion=3.0` 且每项只保留唯一 `canonical` 与必填 `context`；只用于 post-ASR Agent 审查，不进入 ASR context、hotwords 或 prompt。
- `config/transcript-normalization.json` 保存工作区级无条件文字风格统一，例如 `我操 -> 卧槽`。程序在 Agent 审查前对候选字幕执行一次非级联、最长优先的精确替换，记录自动修正审计；Agent 继续审查替换后的完整句子。
...
- `/chronology` 是 downstream truth materialize + review 页：先显式运行 `span-rebuild` span-builder 生成候选 stripped spans，并通过本地 qwen 文本 LM 按 8 个 candidate span 一批生成 provisional 中文 `materialPatterns[]`；LM 返回 ordered pattern rows，代码按 chunk 顺序写回并做 slot 校验，但不再裁决 speech drop、visual-only 或口播时间收缩。Analyze 必须为 keep 的非音频素材产出视觉描述，`span-rebuild` 遇到缺失 report 或缺失 `visualObservation` 必须失败而不是补猜；新 fine-scan report 要让 `fineScanWindows[]` 保存来源 `interestingWindow` 血统和 speech/mixed 窗口的裁剪 transcript truth，`span-rebuild` 优先消费这些窗口自带事实；旧 report 只在 fine-scan window 缺 `semanticKind`、自身或来源证据为 `speech-window` 且与 report transcriptSegments 重叠时保守恢复 speech truth；明确 `semanticKind=visual` 的窗口即使 transcript overlap 也不自动转 speech。新 Analyze 在类型扩窗完成、fine-scan/VLM 前先处理同素材 speech/visual 高重合：以 speech edit-range 集合和每个 visual edit range 的区间集合 `IoU >= 0.5` 为唯一触发条件；触发后扣除 speech，连续 remainder `<15000ms` 删除，`>=15000ms` 裁成独立 visual 候选并留给 `material.recall` 选择。`IoU < 0.5` 的 visual 保持原窗；旧 report 的既有重叠仍按历史事实读取，`span-rebuild` 和 review 不得事后改写。visual 不得继承 transcript。相邻 speech/mixed 候选按同素材、同口播通道、间隔 `<=3000ms`、合并后 `<=45s` 且可见中文 transcript `<=160` 字合并；行车 visual fallback/fine-scan 窗口按 passage 降碎片，默认同素材相邻 gap `<=60s`、单段 `<=90s`，speech/mixed 行车 span 独立保留，visual passage 在 fine-scan 前应用上述重叠消减。`materialPatterns` 失败重试不得走画面事实启发式补写：代码只允许做 JSON 壳解析容错、受控槽位校验、错误原因记录，以及把上次失败原因、期望口播槽值和输出行数约束反馈给 LM；主 chunk 重试后仍失败的 span 会进入单条纠错队列，每项最多自动纠错三次，每次保存最新校验反馈与 partial checkpoint，全部用尽后才阻塞。单条纠错 prompt 只描述固定七槽 schema、当前 item 和机器校验结果，不增加素材案例或按错误类型堆叠指令；第 2、5、6、7 槽的事实短语仍必须来自 LM 或明确的未知规范词。如果存在 speech/mixed candidate span，span-builder 写 `store/spans.json` 后把 meta 标为 `pending-speech-review`，并写 `.tmp/chronology/speech-window-agent-handoff.md`，提示 Codex Agent 生成统一口播与字幕审查产物；Console 按五类分表展示，建议默认接受，只有字幕需听音项保持未决，窗口无法可靠提出动作时保持原状并省略；用户整轮提交后才写最终 fresh spans，且 review 不得事后缩短、删除或切碎旧 report 的 visual span；再运行 `chronology-build` 生成 / 刷新 Chronology V2。页面必须区分 Supervisor live job 和 durable terminal evidence：运行时显示实时进度，失败或阻塞后持续显示最后阶段、错误、失败数、自动重试次数和 checkpoint 续跑入口，直到更新的成功产物或后续任务取代它；`span-rebuild` 进度写入 `.tmp/chronology/progress.json`，已完成部分和 failed span 列表写入 `.tmp/chronology/span-rebuild.partial.json`；`chronology-build` 还必须显示 GPS reverse-geocode 地名解析进度
- `store/spans.meta.json` 记录 `schemaVersion/status/generatedAt/inputsHash/assetCount/reportCount/spanCount/warnings`，并在需要时记录 `speechReview.status / candidateCount / handoffPath / reviewArtifactPath / reportPath / updatedAt`；`pending-speech-review` 表示统一口播与字幕审查尚未提交，Chronology 与 Edit Flow capability 只接受 `status=fresh` 的 spans
...
- `chronology-build` 生成的是确定性候选事件。随后必须经过独立的 Codex Agent 事件语义归并，再开放人工编年史审查。普通模式只合并相邻且地点一致的 `pending event`；Pharos 吸收模式允许一个 confirmed Pharos event 作为唯一锚点，吸收两侧语义属于同一行程的相邻普通 `pending event`，并保留锚点 id、标题、地点和 confirmed 状态。日期零点不是边界，周边 GPS 地点字符串也不必与 Pharos 标签逐字一致；`route`、`gap` 和另一个 Pharos event 仍是硬边界。Agent 只撰写普通合并的标题、合并摘要和理由，不得改 GPS、路线、源时间和 span 内容。程序验证 inputs hash、event 连续性、锚点唯一性以及 span 无遗漏/重复后写回 draft chronology，并把 handoff、状态和前后审计写入 `.tmp/chronology/`
...
- `materialPatterns` 是 span-builder 阶段由 LM 从候选 span 文本事实生成的中文脚本可消费短语数组，不再是 `{ phrase, confidence, evidenceRefs }` 对象数组；prompt 要求每个 candidate span 正好 7 项。前四项固定为 `拍摄视角/构图形态 / 当前环境 / 天气光线 / 口播语音`，其中视角使用受控词表、环境为提取短语、天气光线只写可观察自然现象、口播语音只写 provisional `有口播语音 / 无口播语音`；最终 speech 裁切、取消和 slot4 修正由统一口播与字幕审查的批量提交负责。slot1/slot2/slot3 的画面语义支持性属于 prompt/rubric 约束；代码只校验受控词表、基础结构冲突和错槽保留词（如 `photo + 行车/运动/口播视角`、`drive + 航拍视角`、`无口播语音 + 口播视角`、视角/口播 tag 出现在环境或天气槽），不再用正则对 visualObservation 或标签文本做语义支持性判断。第 5 项是 LLM 给出的短情景故事或 `情景不明`，第 6-7 项是 LLM 给出的短 factual free tags；代码只校验，不把旧词或冲突词启发式替换成新词；`labels`、`report.summary`、GPS / 时间 / Pharos 归属不得写入或传入生成上下文
- `span` 不再持久化旧五轴语义树：`narrativeFunctions / shotGrammar / viewpointRoles / subjectStates / span.evidence`
- `span-rebuild` 只写 `store/spans.json` 与 `store/spans.meta.json`，不写 chronology；如果本地文本 LM 不可用或最终仍有非法七标签行则失败且不改写 spans，并保留 partial checkpoint。失败列表的单条纠错使用独立短 prompt，只保留当前 item、validation feedback、七个槽、期望口播槽、两个不重复自由标签和完整 JSON 外壳，使续跑只补失败项；`chronology-build` 必须先确认 spans fresh，再从 corrected assets + spans + Pharos context 生成 `media/chronology.json`
...
- 对 `source-speech` 而言，Analyze 先从 aligned-token 时间生成自适应 speech windows，不读取字幕句界、文字或字数，也没有固定 gap / 最长时长。窗口扩展完成后、fine-scan/VLM 前，以同素材 speech 区间集合和每个 visual edit range 的 `IoU >= 0.5` 作为唯一重叠触发条件；触发后扣除 speech，连续 remainder `<15000ms` 删除，`>=15000ms` 裁成独立 visual 候选，是否召回仍由 `material.recall` 决定。`span-rebuild` 后续只对已经存在的同 asset、同 speech/mixed 通道候选做保守合并；其 `<=3000ms / <=45s / <=160` 兼容限制不会拆分一个 Analyze speech window。旧 report 已有的 speech/visual 重叠保持只读兼容，后置 review 只能修 speech 候选，不得改写历史 visual span
  - 细粒度语音边界与停顿继续保留在 `transcriptSegments`
  - 历史 speech-boundary 包络/阈值分析只保留诊断能力，明细可写入 `analysis/speech-boundaries/<assetId>.json`，但不得覆盖 Qwen aligned timing 或作为正式口播取源优先级；`effectiveSpeechStartMs / effectiveSpeechEndMs` 仅作无 timed transcriptSegments 时的旧数据兼容字段
...
- 每个 capability 的输出由 `outputRefs` 决定；`timeline.generate` 的正式输出是 Resolve timeline，并同时写 `.tmp/edit-flow/<editId>/timeline/current.srt` 作为按 Resolve 实际落片帧位生成的原声字幕；`.tmp/edit-flow/<editId>/timeline/current.json` 只作为本机临时 KTEP/manifest 审计文件
...
- `/chronology` 顶部使用产物与 live job 驱动的五段横向阶段链：`素材片段与模式 -> 口播与字幕审查 -> 编年史生成 -> 编年史审查 -> Edit Flow`。每段独立显示 `等待中 / 执行中 / 待审查 / 已完成 / 警告 / 失败`、数量或阻塞原因和自己的启动/重试/Agent/审查动作；只有当前可执行段使用主按钮，后续段说明前置条件。重建/刷新等维护动作保留为次级入口，已恢复的历史 LM warning、inputs hash 和 checkpoint 只放在折叠诊断中。
- `materialPatterns` 完整性是 fresh spans 的硬条件：每个 span 必须正好 7 个非空标签。Console 与 `assertFreshSpans()` 都必须报告不完整行数并阻止 `chronology-build`；`span-rebuild` 的 `repair-patterns` 模式只重新请求不完整行，保持其余 span、口播审查结果、窗口和 id 不变，全部修复后才恢复 `fresh`。
...
- ASR 后置文字校对属于 `/chronology` 的 post span-builder“口播与字幕审查”，不属于 Analyze：原始 `asset-reports`、ASR checkpoint、segment 时间和分段保持不变；简体中文正字归一和配置的精确文字归一先写入 candidate spans 并生成审计，Agent 再根据归一后的完整句子、相邻口播、effective glossary 与当前时间段行程生成其他字幕修正和 speech-window 裁切/取消建议。durable audit 为 `analysis/speech-transcript-reviews/<inputsHash>.json`，五类报告分别成表；窗口不确定项按 keep 省略，全部字幕未决项处理并通过原子校验前 meta 保持 `pending-speech-review`，不得运行 `chronology-build`
...
- Analyze 现在按素材分阶段持久化可恢复状态：
  - `analysis/prepared-assets/<assetId>.json` 保存 coarse prepared checkpoint（source file fingerprint / keyframes / `hasAudioTrack` / source context 等输入）；源文件大小或修改时间变化、代表帧为空、代表帧文件丢失或早于源文件时，checkpoint 必须失效并重新粗扫，不能把失败的空代理状态当成可恢复完成态
...
- Analyze 当前正式把项目级 ASR caller 固定为中文优先：
  ...
  - TS 侧先对 ASR 原文与 aligned token 做一致的上下文简体归一，再校验字符覆盖、修复字级长时离群并按确定性标点规则拆句
...
- ASR backend 必须提供带时间的 aligned token；无论底层 Qwen3 + ForcedAligner 或 Whisper 返回字符还是词，写 checkpoint 前都统一展开为 Unicode 单字符 token，并只在原 token 时间范围内分配字符时间
- Kairos 不再由 TS 按停顿、标点或长度重建正式 `transcriptSegments`；Qwen3.5 直接读取 token 和 `gapAfterMs`，返回逐字保真的语义段文本，程序按顺序映射回连续 token range
- `report.transcriptSegments` 当前正式表示经连续 token 覆盖和字级时间异常校验后的确定性 punctuation-gap segmentation，而不是后端粗 segment 或 LLM 输出
...
- `resolve.media_sync` 是 deterministic runner，从 confirmed chronology / fresh spans / assets / root path 映射把事件素材同步进达芬奇 Media Pool；chronology 在这里仅用于 Resolve Media Pool bin 组织和工程归档，不是 `material.recall` 的语义接口；不新增 `media-archive.json`，达芬奇 Media Pool 本身是素材归档真相。重复运行必须复用已有 MediaPoolItem：事件目录一致则跳过，事件目录变化则只移动到新目录不重导入，并在同步结束后清理空事件目录；run record 只记录 imported / reused / moved / pruned 摘要。Resolve bin / item name 与 hostSummary display label 只是人工识别和 debug 回声，不得回流为 Kairos 地点、路线、事件标题或字幕事实。
- `/edit` 还提供项目级 Resolve `[Edit]` 工程维护动作：用户可手动触发素材路径重链，后端按 `project-brief` root 的当前可读路径与 primary/alternate 候选生成映射，Resolve host 精确加载现有 `${projectBrief.name} [Edit]`，核对 `Kairos Project Media` 与目标 edit timeline 后调用 `RelinkClips` 并 `SaveProject()`；若 `project-brief.json.voiceoverMedia` 已配置，同一动作还会按配音媒体主路径 / 备选路径处理 `Kairos Voiceover` Media Pool bin，并在摘要中单独显示配音重链、跳过或警告状态。若 `project-brief.json.audioMedia` 已配置，同一动作还会按该项目外部音频主路径 / 备选路径处理 audio-only 的 `Kairos Audio` Media Pool bin；它是 BGM、SFX 和其它手工加音频的共享目录配置，重链阶段不区分角色。该动作只返回重链/未映射/不可读/缺失目标/跳过非文件/旧路径剩余等摘要，不自动导出 DRP，也不属于 Flow Plan step 执行。Resolve compound/timeline 等非文件 item 不参与重链，缺失本机目标只作为 warning，不阻塞其它可重链素材。
...
- `timeline.generate` 必须通过 Resolve host 创建/更新粗剪 timeline；Resolve 对每段 source range 按素材自身帧率量化，计划毫秒时长不能作为最终字幕时钟。生成器只做一遍原生落片，读取每个 clip 的 `actualStartFrame / actualEndFrame`，把 KTEP clip 重定时到实际帧位，再从已选中、实际有声的 source-speech spans 生成 `.tmp/edit-flow/<editId>/timeline/current.srt`；不得为了字幕再次重建视频时间线，也不得把未执行的 SRT 导入写成成功。最终才写 `.tmp/edit-flow/<editId>/timeline/current.json` 本机临时 KTEP/manifest 审计。航拍视频在粗剪中一律强制禁用 linked audio，即使 ASR 误识别出 transcript 或 `speech/mixed`，也不进入 source-speech handle / SRT 路径。被选中且实际有声的非航拍 speech/mixed 非照片 clip 在落 Resolve 前，优先以有效 `transcriptSegments` 的首末时间作为 Qwen aligned 口播核心，再只扩展一次 source padding：头部 `250ms`、尾部 `250ms`，并 clamp 到素材时长；timed segments 缺失时才依次兼容旧 `effectiveSpeechStartMs/effectiveSpeechEndMs` 与 span source/edit window。SRT 仍按 transcriptSegments 的真实时间映射到扩展后的实际 clip 内部，但 transcript segment 只是时间单元，不保证已有句界仍符合正式规则：保持 36 个文本单位上限，只有 `，` / `,` 是软标点，其余 Unicode 标点一律结束句子；超长句只选择满足上限所需的最少逗号边界并以整体最均衡为准，某个逗号分句后的正文仍超限时才使用字符兜底。同一 clip 内相邻候选的映射间隔 `<1500ms` 且合并后 `<=36` 时必须继续合并；最终 cue 若停在软逗号上，必须把尾逗号提升为 `。`，并在这条 source-speech SRT 中保留该句界。拆出的 cue 在原 segment 时间内按文本单位比例分配，不回写 span 文本或 segment 边界。其他自动旁白字幕、字幕审查稿、最终旁白 SRT/VTT 和配音插件合并文本仍移除行末 `。` / `.`，保留 `？` / `！` 与句内标点。照片默认是 `1000ms` 静帧，只有剪辑规则 / confirmed Flow Plan 或运行时 `timelineStillDurationMs` 显式声明时才改变；Resolve clip color 是正式批处理标记：照片 video item 使用 `Blue`，延时 video item 使用 `Purple`，行车 video item 使用 `Brown`，航拍 video item 使用 `Teal`，没有这些视觉分类颜色的普通有声 video item 使用 `Orange`；剩余可听 linked audio item 使用 `Orange` 供音频归一筛选。生成粗剪还会创建/复用 Resolve Color Groups `Kairos Photos` 与 `Kairos Timelapse`，并把照片/延时 video item 分别分配进去，供 Color 页 `Group Post-Clip` 批量处理。Resolve 不可用、Media Pool 缺素材、source range 回读校验失败、timeline placement 出现空帧、clip color 或 color group 无法写入、图片 still duration 不匹配时不能作为 KTEP-only 成功兜底。
...
- `postlock.subtitle_narration` 的旁白框架阶段必须以当前 Resolve timeline 的字幕轨为口播边界真相，并以 clip-level packet 为唯一时间线事实源：`.tmp/edit-flow/<editId>/postlock/current-timeline-clip-packet.json` 不能预先按 day/event/route/type 合并或丢失 clip 边界。Resolve 在这里只提供锁定 clip 顺序、轨道、timeline/source range、MediaPool source 和 `clipId / assetId / spanId / eventId` 映射；Resolve item name、MediaPool folder name、rough-cut `hostSummary.clips[].eventTitle` 或从 Resolve 名称拆出的显示标签都是 debug/display-only，不得作为地点、路线、事件标题、地貌、水文、植被或字幕写作事实。正式正文使用剪辑规则附录的 Markdown pack-list v2：顶层是 `口播 pack / 行车 pack / 航拍 pack / 照片序列 / 延时 / 延时序列 / 普通视觉` 等叙事单元，`整体` 或 `摘要` 写 pack 级理解，`clips` 保留 leaf clip 描述；行车 / 开车无口播 clip 可以在正文中按相邻连续关系形成 pack，航拍 clip 可以按相邻相关关系形成 pack，照片序列和同编年史事件的延时序列可以合并为一个顶层条目。clip-map 是轻量 v2 边界索引，只允许写 `schemaVersion / format / sourcePacket / entries / packs`；`entries[]` 只存 `marker + clips` 并覆盖所有 Resolve video clip，`packs[]` 只存 `title + entries`，不得复制 packet 中可反查的 `assetIds / spanIds / previousClipIds / summary` 等事实。普通无字幕非照片 clip 在 leaf 层只能归属一次，pack 只作为写作组织和后续字幕审查上下文。无字幕视觉事实只来自 `visualObservation`：生成器不得使用 `materialPatterns` 生成、分类或兜底视觉描述；`semanticKind=speech/mixed` 但当前 Resolve clip 无字幕时，先按当前 item 实际 source range 匹配同 asset 的重叠 visual span，其次匹配 `<=15s` 最近 visual span，最后才使用当前 speech/mixed span 自带 `visualObservation`。无字幕视觉 / 航拍 / 延时正文必须贴近剪辑规则附录，用场景 / 动作短句交代路线、事件和画面推进关系，不得把 `visualObservation` 压缩成 `雨后湿滑、高速路面、道路延伸、车流穿行` 这类逗号标签串。口播 pack 必须从当前字幕内容提炼短摘要，不能只写地点 / 事件 + `口播`，不能粘贴字幕全文，也不能泄漏 `口播信息待人工复核` / `待人工复核` 这类内部占位。口播 clip 只有相邻、时间连续且字幕表达连续或同一话题时才可合并；允许合并时须在 packet 中留下显式 merge group 证据，跨事件边界不能只因字幕片段重叠就合并。正式写入 `edits/<editId>/postlock/narration-framework.md` 前必须生成 `edits/<editId>/postlock/narration-framework.clip-map.json` 并通过 `node scripts/validate-postlock-narration-framework.mjs projects/<projectId> <editId>`，失败不得写成功或 awaiting-review run record。
- `postlock.subtitle_narration` 的旁白框架正文定位只写下游可用的可读地名 / 道路 / 村寨锚点，以及山体、江河湖泊、植被、桥隧和地貌特征；GPS 来源、`GPX±Ns` 匹配误差、`机内GPS`、经纬度、source path、confidence、`�` 和 mojibake 显示串不得进入正文。定位事实只能来自 `media/chronology.json`、GPS/GPX、reverse-geocode cache、precise geo audit 和当前 asset/span，不能从 Resolve item name 或历史 `hostSummary.eventTitle` 反推。行车 leaf 必须按当前 Resolve clip 的源时间中点取空间锚点，不得复用 pack 的整段 `A → B` 路线；航拍可按 confirmed chronology event 组织 pack，但 leaf 仍要回到实际飞行位置和视域。道路、桥梁、隧道、互通、收费站、跨江 / 跨海通道等线性交通设施属于有效可读锚点；项目部、管理中心、停车场、观赏点等 POI 全名只能留在审计数据，正文最多抽取其中真实交通设施名。行车 / 航拍 pack 不得跨 confirmed chronology event 强行合并，`整体` 必须归纳子 clip 的地貌、水文、道路、植被、村镇和天气路况变化，不能只拼接首尾或前三条描述。
- `postlock.subtitle_narration` 的定位括号还必须做地貌 / 水文 / 植被类别去重：已经写出具体河湖海、峡谷崖壁、山口峰丛、桥隧道路或植被带时，不得再追加 `江河湖泊 / 峡谷崖壁 / 林带植被` 等泛类标签。`林带植被` 不得进入正式正文；可确认时要结合 GPS 地点、季节、海拔带和画面线索写成 `干热河谷灌丛、云南松和针阔混交林`、`常绿阔叶林坡`、`高山针叶林、杜鹃灌丛和冷杉林线`、`湖岸草甸` 等可辩护表达，证据不足时省略。
- `postlock.subtitle_narration` 的最终字幕审查稿 `edits/<editId>/postlock/subtitle-review.md` 只有在 `narration-framework.md` 经人工确认后才可生成；审查稿的表格列名、行粒度、跳过规则、占位文本和单元格内 clip 范围标注方式由当前剪辑规则与 confirmed Flow Plan step notes 定义，全局设计不固定项目级表头。审查稿必须参考 Flow Plan 授权的 `styleUsage.literary` 风格档案，并参考当前 Resolve timeline 每个 clip 或 clip 序列组的真实时长；连续相邻的行车或航拍视频可以先聚类判断整体动机，再按剪辑规则要求的粒度编写，首尾 CLIP 可承上启下并参考前后口播但不得覆盖已有口播字幕。普通非照片 clip 默认一条短 caption；只有延时摄影和延时序列允许突破一 clip 一句，且字幕数量不设固定 2-3 条上限，应优先保证画面叙述完整，按天光、云雾、色温、车流 / 人流、山体显露、夜色或晨雾回升等可观察变化拆成足够清楚的多条短字幕，并保留 clip 标签 / 范围或子范围，最终 SRT 按真实 timeline 时间分配。普通行车、航拍和视觉 clip 不因“较长”自动获得拆分例外。照片和照片序列默认不生成旁白字幕；若当前剪辑规则要求保留对应行，按剪辑规则指定的占位文本填写。被聚类且标记为 `开车` 的片段必须结合位置、路线 / 道路名推断与前置视觉分析景观事实来写，无法确定时按剪辑规则指定的方式标注待确认，不得编造。
- Resolve 内直选字幕配音当前通过 `apps/resolve-volc-voiceover-plugin/` 独立实现：插件安装到 DaVinci Resolve `Fusion/Scripts/Edit` 菜单，枚举当前 timeline 的 subtitle tracks，并要求用户在插件面板里指定旁白字幕轨道；多选字幕列表只加载该轨字幕，当前播放头定位、Resolve In/Out 批量选择和 Clear 也只作用于所选轨道，避免把 source-speech / 原声字幕误当旁白合成；Locate 只负责选中并滚动到插件列表行，不追求 Resolve 原生 timeline item 选中。Lua 插件优先通过本地 TCP/HTTP 调用 `127.0.0.1:8940` Supervisor TSV API 获取 profile/config summary 并请求火山引擎 V3 TTS / V3 声音复刻合成；当 Resolve Lua runtime 缺少 socket/TCP 能力时，插件改走 Supervisor 管理的 workspace 文件 IPC（`.tmp/resolve-volc-voiceover-plugin/ipc/requests|responses`），仍由 Supervisor 执行同一 TSV 逻辑。Supervisor 只负责项目匹配、`voiceoverMedia` 选择、runtime profile 读取、direct Volcengine fetch、cache、音频落盘、duration probe 和 manifest 写入，不连接 Resolve；Lua 自己完成 Media Pool bin 创建、音轨选择、`ImportMedia`、`AppendToTimeline`、marker 和 `SaveProject`。单选时一条字幕生成一个 audio item；多选时直接把用户选中的所有字幕按 timeline 时间排序合并成一个 TTS 文本、移除行末 `。` / `.`、生成一个 merged audio item，并插入到第一条所选字幕起点，项目 `.tmp` debug manifest 记录 `isMergedGroup / groupSize / sourceSubtitleIds / sourceSubtitles`。它不是 Flow Plan capability，也不读 Resolve 原生 timeline 多选态，因为官方 scripting API 未提供稳定接口。字幕正文只能来自 Resolve subtitle item 暴露字段；若当前 Resolve 版本不暴露正文，插件必须阻塞并提示导入 SRT 或粘贴文本。插件不再走 `.cmd`、curl 或 Python backend，也不依赖 Python 可执行文件；文件 IPC 只读写 workspace 请求/响应文件，不启动外部命令链。火山 `volcApiKey`、默认 voice profile 和 profiles 注册表写在 workspace 全局 `config/runtime.json.voiceover`，Resolve 面板只通过下拉选择注册 profile，不直接编辑 API key / speaker / resource / model / language。生成音频使用当前 Kairos 项目 `config/project-brief.json.voiceoverMedia` 解析出的可写媒体根；正式媒体目录固定为 `<voiceoverMediaRoot>/<safe Resolve project>/<safe timeline>/`，目录内只保存 Resolve 直接导入的 `vo_<unit>_<request>.mp3`，方便多设备按同一项目配置 relink；`voiceoverMedia.resolveProjectAliases[]` 可显式把临时 / debug Resolve 工程名归属到该项目；请求缓存、debug manifest 和插件日志写到匹配项目的 `.tmp/resolve-volc-voiceover-plugin/`，便于和项目级调试材料一起清理。Resolve 面板只暴露 `Insert`，不再提供只打开系统播放器的 Preview。若当前 Resolve 工程名无法唯一匹配 Kairos 项目，或未配置可写 `voiceoverMedia`，合成必须阻塞。默认不删除已有音频，只有显式 replacement 且命中插件 marker/customData 的旧音频才可删除。
- Resolve Media Pool 的项目全局 `Kairos Project Media` bin 由 `resolve.media_sync` 按 chronology event title 同步，避免 spanId/assetId 一级分组污染人工审查，也避免按 editId 重复归档素材；粗剪 timeline 固定放在 `Kairos Timelines` bin。粗剪创建只走 Resolve 原生 API（当前为 `MediaPool.AppendToTimeline`），不走 FCPXML。当前 `speed > 1` 仅作为待办请求保留，不应用到 Resolve 粗剪；`audio <= -100` 的视频素材保留 linked audio item 并禁用该 audio item，照片可无 audio item，航拍视频一律以 disabled linked audio 落位。照片 video item 设置为 `Blue` 并分配到 `Kairos Photos`，延时 video item 设置为 `Purple` 并分配到 `Kairos Timelapse`，行车 video item 设置为 `Brown`，航拍 video item 设置为 `Teal`；没有这些视觉分类颜色且 `audio > -100` 的普通视频 clip 设置为 `Orange`；剩余可听 linked audio item 设置为 `Orange`，便于用户分别按音频、照片、延时、行车、航拍筛选，并在 Color 页按组批量处理。非 `0 dB` clip gain 必须由 host live probe `TimelineItem.GetProperty()` 并验证可写属性，不能猜 `Volume` key。
...
- `source-speech` 当前正式以“过滤后的口语 transcript cues + merged audio units”作为边界真值：
  ...
  - 时间线当前应优先信任 Analyze 产出的 semantic transcript segments；展示层因版式拆长 cue 只能作为渲染排版，不能写回或改变 transcript review、asset report 与 audio checkpoint 的句界/时间
  - 展示层若拆长 cue，时间码按 cue 长度 / 语速加权映射，而不是整段平均切分
  - `spans.json` 不应为同一段 source-speech material 机械保留多条近邻小 span；Analyze 应优先合并附近 speech spans，把真正的语音细节留在 `transcriptSegments`

### Timeline / Export

- `timeline.generate` 围绕 Resolve rough-cut timeline 展开；`KTEP` 只保留为 `.tmp/edit-flow/<editId>/timeline/current.json` manifest / audit，源语音 SRT 保留为 `.tmp/edit-flow/<editId>/timeline/current.srt`，并由同一步骤在单遍落片后按实际 Resolve 帧位生成。
...
- `material-slots.json.treatments` 是静音/加速唯一正式覆盖来源：缺省读取为 `audio=0,speed=1`，因此默认有声正常速 span 不需要写 entry；被选中的非照片非航拍 span 只要有 transcript、transcriptSegments、`semanticKind=speech/mixed` 或 `materialPatterns=有口播语音` 就禁止写或解析出 `audio<=-100`，这个保护覆盖 `drive / broll / timelapse / talking-head`，不以 `talking-head` 为边界；航拍视频原音在 `timeline.generate` 中一律禁用，即使 ASR 误识别出 speech truth 也不进入原声保护或 source-speech SRT；`coverageAudit` 负责暴露未选入的 protected speech-backed 非照片 span，但代码不得把它们盲目追加进主粗剪时间线，扩召回应回到 `material.recall` 的选择逻辑与人工审查；无口播行车可由 treatment 显式静音/整数倍加速，航拍可显式整数倍加速但音轨仍强制 disabled，且 `speed` 必须在 `1..5` 内；照片默认仍由素材召回写 `{audio:-100}`，时长由 timeline 规则处理。
...
- `timeline.generate` 再从已同步 Media Pool 读取素材，用 Resolve 原生 `AppendToTimeline` 创建/替换粗剪 timeline；静音视频通过禁用 linked audio item 实现，航拍视频总是先强制静音，有声非航拍 speech/mixed 非照片 clip 默认按 timed transcriptSegments 首末边界保留 `250ms` 头部和 `250ms` 尾部 source padding。Resolve clip color 由 host 自动写入并回读校验：照片 video item 为 `Blue`，延时 video item 为 `Purple`，行车 video item 为 `Brown`，航拍 video item 为 `Teal`，没有这些视觉分类颜色的普通有声 video item 为 `Orange`，剩余可听 linked audio item 为 `Orange`。同时创建/复用 `Kairos Photos` 与 `Kairos Timelapse` Resolve Color Groups，并把照片/延时 video item 分别分组。单遍 host 落片返回的实际 clip 帧位用于重定时 KTEP 和 source-speech cues；生成器据此写 `.tmp/edit-flow/<editId>/timeline/current.srt`，通过 source range / still duration / timeline placement / clip color / color group 校验后写临时 `.tmp/edit-flow/<editId>/timeline/current.json` 审计。没有 transcriptSegments 时才用整段 `transcript` 兜底。SRT cue 文本经过共享字幕 helper，统一移除行末 `。` / `.`，但保留问号、叹号和句内标点。Resolve 不可用、Media Pool 缺素材、原生 append/静音失败、source range 错误、图片时长不匹配、timeline placement 出现空帧、clip color/color group 无法写入或非 0 dB clip gain 无法 live probe 设置时直接阻塞。当前 `speed>1` 不阻塞，但会被忽略并记录为 pending。
- 字幕当前分为粗剪伴随 SRT 与 post-lock 文本路径，所有自动生成路径共享“行末不补 / 不保留 `。` 或 `.`”规则：
  - 粗剪伴随路径：`timeline.generate` 从已选中、实际有声且非航拍的 source-speech spans 按 Resolve 实际 clip 帧位写 `.tmp/edit-flow/<editId>/timeline/current.srt`
  - post-lock 旁白路径：默认来自 `beat.text`
  - post-lock 原声路径：当某拍保留原声时，来自 `beat.audioSelections[]` 对应的 merged audio units
- post-lock 最终字幕审查稿不是素材说明书：若用户在当前修稿任务中提供或明确认可旁白样本，应先读取它作为本轮修稿参考；普通素材参考样本只是当前任务的风格修正手段，不自动沉淀为后续永久规则。普通非照片 clip 默认保留一条可落地短 caption，只在剪辑规则、Flow Plan notes 或用户明确允许时合并相邻普通 clip；只有延时摄影 / 延时序列可突破一 clip 一句，必须按画面内部时间变化、天气天光、云雾推进、交通流动、山体显露、夜色或晨雾回升拆成足够完整的多条短字幕。修订时尽量稳定表格身份列和事实概要列，只改写旁白字幕列，并用 audit 校验行数、生成 / 不生成数量、clip 标签、同 clip 拆分、合并跨度和 GPS / 坐标 / mojibake / 泛类地理标签噪音。
...
- deterministic timeline 不再有段级 Agent reviewer；召回回退、跨段换料、非 `drive/aerial` 加速、dropped asset 进入召回、有 speech truth 的非照片非航拍 span 被静音，以及需要静音的照片/素材未写 treatment 覆盖等问题由 `material-slots` contract 和 timeline build 前置校验直接阻塞；航拍音轨由 timeline build 统一强制 disabled。
- `timeline.generate` 当前默认按 span 的自然 source 时长 / edit-friendly bounds 摆放普通视觉 clip；有声非航拍 speech/mixed 非照片 clip 则优先以 timed transcriptSegments 的首末时间作为 Qwen aligned 原声核心窗口，再在外侧只加一次 head/tail padding（`250ms` / `250ms`，受素材边界约束）。旧 `effectiveSpeechStartMs/effectiveSpeechEndMs` 只在 timed segments 缺失时兼容读取，响度分析不得覆盖对齐边界。航拍 clip 强制静音，不加 source-speech handle。`speed` 来自 `material-slots.json.treatments` 覆盖值，缺省为 `1`；读取时会把 raw speed 规整为 `1..5` 的整数倍。
- 已人工剪过前缀时，`timeline.generate` 可显式使用 `resume-from-current-playhead`：严格核对既有 Resolve project/timeline，以播放头命中的完整 video item 末帧作为保留边界，并从 item name 中读取稳定 `clip-xxxxx` 计划锚点。边界之前的人工 video/audio/subtitle 原样保留，边界之后的旧自动后缀删除，再按当前确定性计划和最新 source-window 规则只追加锚点后的 clips。后缀字幕根据本次 Resolve 实际落片的绝对帧位写到 `.tmp/edit-flow/<editId>/timeline/remaining.srt`，不自动导入，也不改写前缀字幕；操作期间使用临时 timeline duplicate 保护失败恢复。
- 对规则升级前已经落入 `store/spans.json / material-slots.json` 的同源 speech/visual 重复选择，时间线落片前执行与 Analyze 相同的兼容过滤：按最终计划 source range 计算区间集合 IoU，`IoU >= 0.5` 时从 visual 扣除 selected speech union，连续 visual remainder `<15000ms` 删除，`>=15000ms` 才裁成实际 visual clip。每个 selected span 的基础 `clip-xxxxx` 编号在过滤前确定，drop 不挪动后续编号，split remainder 使用该基础编号的稳定后缀。
...
- 可恢复中间态：`analysis/prepared-assets/` 与 `analysis/audio-checkpoints/` 用于跨进程恢复 Analyze；它们是 durable resume cache，不是 Edit Flow 的正式输入，且在 stage 语义调整、源文件替换、代表帧为空或代表帧文件丢失后必须安全失效并重建。照片代理生成失败必须保留底层媒体工具错误，不能落盘空 prepared checkpoint，也不能误报为 VLM 缺少视觉描述

### 当前稳定约定

- `config/project-brief.json` 保存项目级素材 root 单真值，包括主路径、原始路径和有序备选路径；项目级 `voiceoverMedia` 与 `audioMedia` 也使用同样的主路径 / 备选路径候选模型，其中 `audioMedia` 是 BGM、SFX 和其它手工加音频的共享外部音频目录
...
- Analyze 新 job 的续跑阶段不读取 `progress.json.step` 作为恢复指针，而是重新从 `analysis/asset-reports`、`analysis/prepared-assets` 与 `analysis/fine-scan-checkpoints` 推导待办；prepared checkpoint 还必须通过源文件 fingerprint 与代表帧完整性校验，失败时重新粗扫。`store/spans.json` 不参与待 fine-scan 判断。如果 coarse report 已全部存在且只剩 fine-scan，首个 live progress 应直接显示 `fine-scan-prefetch`，不能把监控页误拉回 `prepare`。
