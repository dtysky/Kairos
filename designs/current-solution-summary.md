...
- Resolve 内直选字幕配音当前通过 `apps/resolve-volc-voiceover-plugin/` 独立实现：插件安装到 DaVinci Resolve `Fusion/Scripts/Edit` 菜单，扫描当前 timeline 的 subtitle tracks，在插件面板中多选字幕，或按当前播放头 / Mark In-Out 选择字幕，调用火山引擎 V3 TTS / V3 声音复刻，并把每条字幕对应的一条配音音频插入 `Kairos VO` 音频轨。它不依赖 `/edit` Console 或 Supervisor，不是 Flow Plan capability，也不读 Resolve 原生 timeline 多选态，因为官方 scripting API 未提供稳定接口。字幕正文只能来自 Resolve subtitle item 暴露字段；若当前 Resolve 版本不暴露正文，插件必须阻塞并提示导入 SRT 或粘贴文本。生成音频、缓存和 `manifest.json` 默认写到 `~/Movies/KairosVoiceover/<resolveProject>/<timelineId>/<runId>/`；API key、speaker profile 和声音授权只写本机 local config。默认不删除已有音频，只有显式 replacement 且命中插件 marker/customData 的旧音频才可删除。
