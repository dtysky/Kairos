...
- Resolve 内直选字幕配音当前通过 `apps/resolve-volc-voiceover-plugin/` 独立实现：插件安装到 DaVinci Resolve `Fusion/Scripts/Edit` 菜单，扫描当前 timeline 的 subtitle tracks，在插件面板的多选字幕列表中选择字幕，或用当前播放头定位并选中对应字幕、用 Resolve In/Out 批量选择范围字幕、用 Clear 清空选择，调用火山引擎 V3 TTS / V3 声音复刻，并把配音音频导入 Media Pool bin `Kairos Voiceover / <timelineName>` 后插入 `Kairos VO` 音频轨。单选时一条字幕生成一个 audio item；多选时直接把用户选中的所有字幕按 timeline 时间排序合并成一个 TTS 文本、生成一个 merged audio item，并插入到第一条所选字幕起点，项目 `.tmp` debug manifest 记录 `isMergedGroup / groupSize / sourceSubtitleIds / sourceSubtitles`。它不依赖 `/edit` Console 或 Supervisor，不是 Flow Plan capability，也不读 Resolve 原生 timeline 多选态，因为官方 scripting API 未提供稳定接口。字幕正文只能来自 Resolve subtitle item 暴露字段；若当前 Resolve 版本不暴露正文，插件必须阻塞并提示导入 SRT 或粘贴文本。火山 `volcApiKey`、默认 voice profile 和 profiles 注册表写在 workspace 全局 `config/runtime.json.voiceover`，Resolve 面板只通过下拉选择注册 profile，不直接编辑 API key / speaker / resource / model / language。生成音频使用当前 Kairos 项目 `config/project-brief.json.voiceoverMedia` 解析出的可写媒体根；正式媒体目录固定为 `<voiceoverMediaRoot>/<safe Resolve project>/<safe timeline>/`，目录内只保存 Resolve 直接导入的 `vo_<unit>_<request>.mp3`，方便多设备按同一项目配置 relink；`voiceoverMedia.resolveProjectAliases[]` 可显式把临时 / debug Resolve 工程名归属到该项目；请求缓存、Lua/Python bridge job、backend 输出、debug manifest 和插件日志写到匹配项目的 `.tmp/resolve-volc-voiceover-plugin/`，便于和项目级调试材料一起清理。Resolve 面板只暴露 `Insert`，不再提供只打开系统播放器的 Preview；后端 preview mode 仅保留为命令行调试入口。若当前 Resolve 工程名无法唯一匹配 Kairos 项目，或未配置可写 `voiceoverMedia`，合成必须阻塞。默认不删除已有音频，只有显式 replacement 且命中插件 marker/customData 的旧音频才可删除。
