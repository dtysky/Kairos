...
- `/chronology` 是 downstream truth materialize + review 页：先显式运行 `span-rebuild` span-builder 生成候选 stripped spans，并通过本地 qwen 文本 LM 按 8 个 candidate span 一批生成 provisional 中文 `materialPatterns[]`；LM 返回 ordered pattern rows，代码按 chunk 顺序写回并做 slot 校验，但不再裁决 speech drop、visual-only 或口播时间收缩。Analyze 必须为 keep 的非音频素材产出视觉描述，`span-rebuild` 遇到缺失 report 或缺失 `visualObservation` 必须失败而不是补猜；新 fine-scan report 要让 `fineScanWindows[]` 保存来源 `interestingWindow` 血统和 speech/mixed 窗口的裁剪 transcript truth，`span-rebuild` 优先消费这些窗口自带事实；旧 report 只在 fine-scan window 缺 `semanticKind`、自身或来源证据为 `speech-window` 且与 report transcriptSegments 重叠时保守恢复 speech truth；明确 `semanticKind=visual` 的窗口即使 transcript overlap 也不自动转 speech。同一素材内 speech/mixed 与 visual span 可以时间重叠共存：speech 只承载裁剪口播真相，visual 必须保留完整视觉窗口和 `visualObservation`，不得因为 speech 重叠被切断或继承 transcript。相邻 speech/mixed 候选按同素材、同口播通道、间隔 `<=3000ms`、合并后 `<=45s` 且可见中文 transcript `<=160` 字合并；行车 visual fallback/fine-scan 窗口按 passage 降碎片，默认同素材相邻 gap `<=60s`、单段 `<=90s`，speech/mixed 行车 span 仍独立并可与 visual passage 重叠。`materialPatterns` 失败重试不得走画面事实启发式补写：代码只允许做 JSON 壳解析容错、受控槽位校验、错误原因记录，以及把上次失败原因 / 期望口播槽值 / 输出行数约束反馈给 LM 重新生成；第 2、5、6、7 槽的事实短语仍必须来自 LM 或明确的未知规范词，重试后仍不合格则本次 rebuild 失败。如果存在 speech/mixed candidate span，span-builder 写 `store/spans.json` 后把 meta 标为 `pending-speech-review`，并写 `.tmp/chronology/speech-window-agent-handoff.md`，提示用户去 Codex/Agent 启用 SubAgents 做最终 speech-window 裁切并写回 fresh spans；speech-window review SubAgent shard 上限约 1500 candidates，主 Agent 合并所有 shard 后才写最终文件，且 review 不得缩短、删除或切碎重叠 visual span；再运行 `chronology-build` 生成 / 刷新 Chronology V2。页面必须显示 active job 进度，`span-rebuild` 进度写入 `.tmp/chronology/progress.json`，包含 chunk、失败列表补处理、retry/warning 摘要；已完成部分和 failed span 列表写入 `.tmp/chronology/span-rebuild.partial.json`；`chronology-build` 还必须显示 GPS reverse-geocode 地名解析进度
...
- 对 `source-speech` 而言，Analyze 与 `span-rebuild` 都使用同一套口播合并默认：同 asset、speech/mixed 通道、相邻间隔 `<=3000ms`、合并后 `<=45s` 且可见中文 transcript `<=160` 字；超过停顿、长度或文本上限时保留独立 speech span。speech/mixed 与 visual 可同素材重叠共存，后置 review 只能修 speech 候选，不得切碎重叠 visual span
  - 细粒度语音边界与停顿继续保留在 `transcriptSegments`
  - analysis 阶段可追加 speech-boundary 补充分析：每个素材的包络/阈值诊断明细写入 `analysis/speech-boundaries/<assetId>.json`，正式消费只在 `store/spans.json` 的 speech/mixed span 上保留 `effectiveSpeechStartMs / effectiveSpeechEndMs` 两个 source-ms 字段；它们表示经音频分析修正后的真实口播首尾，不替换 `sourceInMs/sourceOutMs`、`editSourceInMs/editSourceOutMs` 或 transcriptSegments
...
- `timeline.generate` 必须通过 Resolve host 创建/更新粗剪 timeline；Resolve 成功后写 `.tmp/edit-flow/<editId>/timeline/current.json` 本机临时 KTEP/manifest 审计，并从已选中、实际有声的 source-speech spans 生成 `.tmp/edit-flow/<editId>/timeline/current.srt`，供用户手动导入达芬奇。需要把既有/迁移 SRT 预载到 Resolve timeline 时，host 必须先生成带内容 hash 的唯一 SRT 导入副本，避免 Resolve 对同一路径复用旧 Media Pool 字幕素材。航拍视频在粗剪中一律强制禁用 linked audio，即使 ASR 误识别出 transcript 或 `speech/mixed`，也不进入 source-speech handle / SRT 路径。被选中且实际有声的非航拍 speech/mixed 非照片 clip 在落 Resolve 前优先使用 `effectiveSpeechStartMs/effectiveSpeechEndMs` 作为有效口播首尾，再默认扩展 source handle：头部 `300ms`、尾部 `300ms`，并 clamp 到素材时长；没有有效 speech-boundary 字段时回退到 span 的 source/edit window。SRT 仍按 transcriptSegments 的真实时间映射到扩展后的 clip 内部。所有生成字幕、SRT/VTT、字幕审查稿和配音插件合并文本都不得自动补或保留行末 `。` / `.` 句号；`？` / `！` 与句内标点保留。照片默认是 `1000ms` 静帧，只有剪辑规则 / confirmed Flow Plan 或运行时 `timelineStillDurationMs` 显式声明时才改变；Resolve clip color 是正式批处理标记：照片 video item 使用 `Blue`，延时 video item 使用 `Purple`，行车 video item 使用 `Brown`，航拍 video item 使用 `Teal`，没有这些视觉分类颜色的普通有声 video item 使用 `Orange`；剩余可听 linked audio item 使用 `Orange` 供音频归一筛选。生成粗剪还会创建/复用 Resolve Color Groups `Kairos Photos` 与 `Kairos Timelapse`，并把照片/延时 video item 分别分配进去，供 Color 页 `Group Post-Clip` 批量处理。Resolve 不可用、Media Pool 缺素材、source range 回读校验失败、timeline placement 出现空帧、clip color 或 color group 无法写入、图片 still duration 不匹配时不能作为 KTEP-only 成功兜底。
...
- `timeline.generate` 再从已同步 Media Pool 读取素材，用 Resolve 原生 `AppendToTimeline` 创建/替换粗剪 timeline；静音视频通过禁用 linked audio item 实现，航拍视频总是先强制静音，有声非航拍 speech/mixed 非照片 clip 默认保留 `300ms` 头部和 `300ms` 尾部 source handle。Resolve clip color 由 host 自动写入并回读校验：照片 video item 为 `Blue`，延时 video item 为 `Purple`，行车 video item 为 `Brown`，航拍 video item 为 `Teal`，没有这些视觉分类颜色的普通有声 video item 为 `Orange`，剩余可听 linked audio item 为 `Orange`。同时创建/复用 `Kairos Photos` 与 `Kairos Timelapse` Resolve Color Groups，并把照片/延时 video item 分别分组。Resolve host 成功创建/更新粗剪 timeline 且 source range / still duration / timeline placement / clip color / color group 回读校验通过后才写临时 `.tmp/edit-flow/<editId>/timeline/current.json` 审计，并从有声非航拍 source-speech clip 的 `transcriptSegments` 映射出 `.tmp/edit-flow/<editId>/timeline/current.srt`；没有分段时才用整段 `transcript` 兜底。SRT cue 文本经过共享字幕 helper，统一移除行末 `。` / `.`，但保留问号、叹号和句内标点。Resolve 不可用、Media Pool 缺素材、原生 append/静音失败、source range 错误、图片时长不匹配、timeline placement 出现空帧、clip color/color group 无法写入或非 0 dB clip gain 无法 live probe 设置时直接阻塞。当前 `speed>1` 不阻塞，但会被忽略并记录为 pending。
- 迁移既有 SRT 到 Resolve rough-cut 时，host 必须在清空/新建 timeline 后、append 视频素材前导入 SRT；Resolve 对已含视频的 timeline append SRT 会把字幕追加到末尾，`recordFrame` 对 SRT 不可靠。
- 字幕当前分为粗剪伴随 SRT 与 post-lock 文本路径，所有自动生成路径共享“行末不补 / 不保留 `。` 或 `.`”规则：
  - 粗剪伴随路径：`timeline.generate` 从已选中、实际有声且非航拍的 source-speech spans 写 `.tmp/edit-flow/<editId>/timeline/current.srt`，供手动导入达芬奇
...
- `timeline.generate` 当前默认按 span 的自然 source 时长 / edit-friendly bounds 摆放 clip；有声非航拍 speech/mixed 非照片 clip 如果有 `effectiveSpeechStartMs/effectiveSpeechEndMs`，先以这两个分析后的有效口播首尾作为原声核心窗口，再在外侧加默认 head/tail handle（`300ms` / `300ms`，受素材边界约束），避免 ASR 固定偏差把口播前后留白倒置。航拍 clip 强制静音，不加 source-speech handle。`speed` 来自 `material-slots.json.treatments` 覆盖值，缺省为 `1`；读取时会把 raw speed 规整为 `1..5` 的整数倍。
