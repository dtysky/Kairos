...
2. Edit Flow 的素材召回与 Resolve rough-cut 生成已消费 transcript 证据
   ...
   - `material-slots.json.treatments` 是召回阶段向粗剪传递音频/速度建议的唯一正式覆盖字段；缺少 `treatments[spanId]`、`audio` 或 `speed` 时按默认 `{audio:0,speed:1}` 读取，因此默认有声正常速 span 不写 entry；被选中的非照片 span 只要有 transcript、transcriptSegments、`semanticKind=speech/mixed` 或 `materialPatterns=有口播语音` 就不得静音，保护覆盖 `drive / broll / aerial / timelapse / talking-head`；未选入的 speech-backed 非照片 span 由 `coverageAudit` 暴露，代码不得盲目追加到主粗剪时间线；无口播视觉 span 需要静音时显式写 `audio=-100`，只有 `drive / aerial` 可显式 `speed>1`，且 `speed` 必须是 `1..5` 的整数倍
...
- Resolve rough cut 会写入正式 clip color 批处理标记：普通有声视频 item 及其 linked audio item(s) 为 `Orange`，照片 video item 为 `Blue`，延时 video item 为 `Purple`；若延时 clip 有 linked audio 且未静音，video item 保持 `Purple`、linked audio item 为 `Orange`，这样照片/延时可用于统一套 `Blanking Fill` 等视觉处理，音频仍可按 `Orange` 归一化。粗剪生成还会创建/复用 Resolve Color Groups `Kairos Photos` 与 `Kairos Timelapse`，把照片/延时 video item 分别分组，供 Color 页 `Group Post-Clip` 批量效果使用。不能用 Color-page filter 或 flag 代替 clip color，因为 Color 页选择不会同步成 Edit 页 timeline selection，Resolve flag 也会沿 linked items 扩散，无法精确表达这些批处理集合。
...
读取 Flow Plan 声明的前序产物
  ...
  → official Python host 先通过 `resolve.media_sync` 复用/移动/导入达芬奇 Media Pool 的项目全局 `Kairos Project Media` 素材：目标事件目录一致则复用跳过，目录变化则移动，缺失才导入，并在同步结束后清理空事件目录；随后在 `Kairos Timelines` 中通过 Resolve 原生 `MediaPool.AppendToTimeline` 创建/替换 Resolve rough-cut timeline，自动写入并回读校验 `Orange/Blue/Purple` clip color 批处理标记，创建/复用 `Kairos Photos` / `Kairos Timelapse` Color Groups 并分配照片/延时 video item，同时回读校验 source range / still duration；有声 speech/mixed 非照片 clip 的回读 source range 包含默认 `240ms` head / `720ms` tail handle，照片默认校验 `1000ms`，只有剪辑规则 / confirmed Flow Plan 或 `timelineStillDurationMs` 可覆盖
...
- `material-slots.json.treatments` 是音量/加速唯一正式覆盖来源：缺省 `{audio:0,speed:1}`，默认字段可省略；`audio` 单位 dB，静音 `-100`；非照片 span 只要有 transcript、transcriptSegments 或 `semanticKind=speech/mixed` 就不得静音；`speed` 单位倍速，但只能写 `1..5` 的整数倍
...
- source-speech SRT 的 cue 时间仍来自 transcriptSegments；speech/mixed clip source handle 只扩展实际 Resolve clip 边界，不把字幕铺满前后静音余量；cue 文本必须经过共享字幕 helper 移除行末 `。` / `.`，但保留 `？` / `！`。
