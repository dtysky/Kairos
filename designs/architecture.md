...
- `span-rebuild`：现在是 span-builder。它只读取 `store/assets.json` 与 `analysis/asset-reports/*.json`，先确定性切片并校验所有 keep 的非音频素材都有 report、所有 material span 都有 Analyze 产出的 `visualObservation`；历史 recognized `speech-window` fine-scan window 若缺 `semanticKind`，必须从匹配 `interestingWindows` 或重叠 transcript 恢复 speech truth；同一素材内 speech/mixed 与 visual span 是可重叠的双通道事实，speech 只承载裁剪口播 truth，visual 必须保留完整视觉窗口和 `visualObservation`，不得被口播窗口切碎或继承 transcript；相邻 speech/mixed 候选按同素材、同口播通道、间隔 `<=3000ms`、合并后 `<=45s` 且可见中文 transcript `<=160` 字合并；行车 visual fallback/fine-scan 窗口按 passage 降碎片，默认同素材相邻 gap `<=60s`、单段 `<=90s`，speech/mixed 行车 span 仍独立并可与 visual passage 重叠；通过本地 qwen 文本 LM 从每个 span 的 `type / semanticKind / transcript / visualObservation` 按素材时间顺序 8 个 span 一批生成 provisional 中文 `materialPatterns[]`；LM prompt 请求按输入顺序排列的 7-tag 短语行，代码按 chunk 顺序写回 span，并严格校验前四个槽；第 1 项是素材自身可观察的拍摄视角/构图形态，不得重复 `type` 的照片/视频载体语义，也不得写“建场/记录/成果”等后续剪辑用途；任一必需槽缺失或冲突都进入 failed span 列表并在主 chunk 后单条补处理，补处理仍不合格则阻塞本次 rebuild，不做启发式改写或兼容映射；已完成 checkpoint 和 failed span 列表写到 `.tmp/chronology/span-rebuild.partial.json`。若存在 speech/mixed candidate spans，收口后写 `store/spans.json` 与 `store/spans.meta.json(status=pending-speech-review)`，并写 `.tmp/chronology/speech-window-agent-handoff.md` 交给 Codex/Agent SubAgents 完成最终 speech-window 裁切；每个 speech review SubAgent shard 上限约 1500 candidates，主 Agent 合并 shard 后才可写最终 fresh spans，且 speech review 不得缩短、删除或切碎重叠 visual span；无 speech/mixed candidates 时可直接写 `status=fresh`。analysis 阶段的 speech-boundary 补充分析可在 span 已存在后运行：每个素材的诊断明细写入 `analysis/speech-boundaries/<assetId>.json`，最终只把 `effectiveSpeechStartMs/effectiveSpeechEndMs` 两个 source-ms 字段回填到 `store/spans.json`，不覆盖 source/edit window 或 transcriptSegments。
- `chronology-build`：要求 spans 存在且 `status=fresh`，`pending-speech-review` 必须先由 Agent speech-window review 完成；再从 corrected assets + fresh spans + Pharos context + 项目 GPX/derived track 生成 Chronology V2 `media/chronology.json`；生成时先归属 Pharos 单点 actual window，再按结构化素材类型、时间连续性和 GPS 轨迹连续性聚合剩余 spans
...
1. `coarse-first analyze` 已把 ASR 纳入视频细扫前链路
   ...
   - `/chronology` 的 `span-rebuild` 对 speech/mixed 使用同 asset、同口播通道、间隔 `<=3000ms`、合并后 `<=45s` 且 transcript `<=160` 字的合并规则；visual 与 speech/mixed 可重叠共存，行车 visual 按 passage 降碎片；细粒度停顿继续由 `transcriptSegments` 表达。Analyze 的 speech-boundary 补充分析只追加 `effectiveSpeechStartMs/effectiveSpeechEndMs` 两个有效口播首尾 ms 字段，完整包络诊断留在 `analysis/speech-boundaries/<assetId>.json`
...
- source-speech audio units 默认按 `<= 3000ms` gap 合并，并在合法范围内保留 `120ms` / `180ms` breathing；Resolve rough cut 的 source-speech clip 先读 `effectiveSpeechStartMs/effectiveSpeechEndMs` 作为有效口播核心窗口，再在外侧加 `300ms` / `300ms` source handle，缺失字段时才回退到 span source/edit window
...
- `materialPatterns` 是 span-builder 阶段由 LM 从最小 span 文本事实生成的中文脚本可消费短语数组，不再是 `{ phrase, confidence, evidenceRefs }`；生成上下文只允许 `type / semanticKind / transcript / visualObservation`，不传 span id、`labels`、`report.summary`、GPS / 时间 / Pharos 归属；`span-rebuild` 当前按素材时间顺序 8 个 span 一批调用本地 qwen 文本 LM，prompt 要求 `string[][]` ordered rows 且每行正好 7 项，代码按 chunk 顺序拼回 candidate spans，并严格校验前四个槽：受控 `拍摄视角/构图形态`、提取 `当前环境`、可观察自然 `天气光线`、provisional 二元 `口播语音`；第 1 项不得重复 photo/video 等载体类型或写后续剪辑用途，slot1/slot2/slot3 是否被画面事实支持由 LM prompt/rubric 约束，代码只用结构字段和错槽保留词拦截硬冲突（如 `photo + 行车/运动/口播视角`、`drive + 航拍视角`、`无口播语音 + 口播视角`、视角/口播 tag 出现在环境或天气槽），不得用关键词正则二次判定支持性；第 5 项是 LM 情景故事或 `情景不明`，第 6-7 项必须来自 LM；代码不得启发式补写、替换旧词或兼容迁移。允许的稳健性修复只限协议层：解析本地 LM 输出时可提取第一段完整 JSON 或补齐明显缺失的 JSON 外壳，校验失败时记录具体 slot issue，并在第二次 chunk 尝试和 failed-list 单条补处理时把失败原因、期望口播槽值和 expected row count 反馈给 LM 重新生成；代码仍不得根据 `visualObservation` 自行创造环境、故事或 free tags。重试后仍不合格则本次 rebuild 失败；必须在 `.tmp/chronology/progress.json` 写入切片、chunk LM、failed-list、ETA、重试、warning 与最终写入进度，并在 `.tmp/chronology/span-rebuild.partial.json` 写入已完成 materialPatterns checkpoint 和 failed span 列表；存在 speech/mixed candidate 时，`store/spans.json` 是 pending candidate truth，必须由后置 Codex/Agent SubAgents 完成 speech-window 裁切并写回 fresh spans 后才可被 chronology-build 消费
...
- source-speech SRT 的 cue 时间仍来自 transcriptSegments；speech/mixed clip source handle 只扩展实际 Resolve clip 边界，不把字幕铺满前后静音余量；航拍 clip 因强制静音不进入 source-speech SRT；cue 文本必须经过共享字幕 helper 移除行末 `。` / `.`，但保留 `？` / `！`。
