...
- Resolve rough cut 会把每个非静音视频 timeline item 及其 linked audio item(s) 设置为 `Orange` clip color，作为用户在 Edit/Fairlight Timeline Index 中筛选后手动执行音频归一化的正式标记；静音 clip 和照片保持无色，不能用 Color-page filter 或 flag 代替，因为 Color 页选择不会同步成 Edit 页 timeline selection，Resolve flag 也会沿 linked items 扩散，无法精确表达非静音集合。
...
- `postlock.subtitle_narration` 的最终字幕审查稿 `edits/<editId>/postlock/subtitle-review.md` 必须在 `narration-framework.md` 人工确认后生成；表格列名、行粒度、跳过规则、占位文本和单元格内 clip 范围标注方式由当前剪辑规则与 confirmed Flow Plan step notes 定义，全局架构不固定项目级表头。审查稿必须参考 Flow Plan 授权的 `styleUsage.literary` 风格档案，并参考当前 Resolve timeline 每个 clip 或 clip 序列组的真实时长；连续相邻的行车或航拍视频可以先聚类判断整体动机，再按剪辑规则要求的粒度编写，首尾 CLIP 可承上启下并参考前后口播但不得覆盖已有口播字幕。照片和照片序列默认不生成旁白字幕；若当前剪辑规则要求保留对应行，按剪辑规则指定的占位文本填写。被聚类且标记为 `开车` 的片段必须结合位置、路线 / 道路名推断与前置视觉分析景观事实来写，无法确定时按剪辑规则指定的方式标注待确认，不得编造。
