...
## 0.22 2026-09-05 ASR 字级时间轴与确定性字幕拆句合同

- `/asr` 的正式输出是原始识别文本、ForcedAligner token 和运行 timing。无 ForcedAligner token 的 rawText 只作审计，不能生成字幕或推测时间。
- `transcript-segmentation` 位于全量 `audio-analysis` 之后、`finalize` 之前，不加载文本模型。它先校验 aligned token 的字符覆盖、索引、时间单调性和 gap 一致性，再以单素材 token 时长分布识别 ForcedAligner 吞入长静音的离群 token；异常区间按邻接标点方向收缩到本素材正常字时长尺度，重算 gap，并在 `transcriptSegmentation.timingValidation` 保存阈值和逐 token 原始/修复时间。修复不得改变字符、索引、顺序或 ASR 原文。
- 正式字幕句界直接消费 ASR 原始标点。中文 `，` 与英文 `,` 是仅有的软标点，其余 Unicode 标点一律结束当前句；连续标点形成一个边界。超过 36 个非标点字符的句子只在已有逗号处选择均衡分割，并把选中的逗号投影为句号；随后仅当相邻句 token gap `<1500ms` 且合并后仍 `<=36` 字时顺序合并，保留内部标点。校验器要求最终 segment 连续、无遗漏、无重叠地覆盖全部 aligned token，segment 时间只由首尾 token 派生。
- audio checkpoint schema v3 同时承载 `alignmentContractVersion=qwen3-character-alignment-v2` 和 pending/completed segmentation 状态，使 ASR 和拆句都可恢复；ForcedAligner 的中文、英文和数字输出统一按 Unicode 字符展开，在原 token 时间范围内单调分配字符时间。有效对齐序列保留零时长弱字，并要求 token 非标点正文完整覆盖 ASR 原文。asset report 保留 raw/token/segmentation 审计字段。下游 transcript review 固定句界与时间，只改文字。
- Windows Supervisor 管理仅绑定 loopback 的独立 Qwen ASR worker：优先使用 `127.0.0.1:8911`，启动前通过真实 socket bind 探测可用性，若被其它程序占用则分配可用临时端口，并把本轮实际 URL 注入 `8910` 主 ML gateway。gateway 代理 ASR health/request/unload/shutdown；全量 ASR 完成后 worker 先卸载模型再退出，主 VLM 环境与 `qwen-asr==0.0.6` 的 Transformers 依赖物理解耦。Supervisor 只停止本轮记录且经 health/PID 验证的 worker，不终止端口上的外部进程。
- ML gateway 与 Analyze monitor 对 ASR 暴露显式生命周期：`ready` 表示 worker 可接收 ASR，`released` 表示本轮字级 ASR 已完成且 worker 已按计划退出，`unavailable` 才表示所需 worker/模型不可用，`not-running` 表示整个 ML 服务未启动。活跃 Analyze 已进入确定性字幕拆句、finalize 或 fine-scan 时，worker 连接拒绝应结合当前真实阶段解释为 `released`，Console 不得显示红色阻塞，也不得因此尝试重启 worker。
...
## 0.19 2026-09-06 口播与字幕统一审查

post span-builder 审查现在是单一的项目闸门：`candidate spans -> Agent 生成建议 -> 用户在 /chronology 审查 -> 原子提交 -> fresh spans`。Agent 阶段不得直接把裁切、取消口播或非正字归一的文本修改写入 `store/spans.json`。

- 结构化事实为 `analysis/speech-transcript-reviews/<inputsHash>.json`，包含 immutable baseline 引用、Agent 建议、用户选择和审计状态；Markdown 仅由同一 JSON 生成，按类别输出独立表格。
- 正式类别只有五个：字幕的 `auto-normalized / suggested-correction / needs-listening`，口播窗口的 `suggested-trim / suggested-cancel`。无变化项完全省略。窗口 Agent 的 `needs-human` 表示没有足够把握提出动作，程序按 keep 处理并省略审查项；旧 `speech-window-needs-listening` 在读取时按 keep 丢弃，不再阻塞人工审查。
- 字幕 `needs-listening` 行通过只读 `GET /api/projects/:projectId/speech-transcript-review/audio/:assetId?startMs=&endMs=` 获取区间音频。客户端在正式审查区间前后各扩展 `1000ms`，起点不早于素材 `0ms`，终点由服务端按素材时长截断；试听余量不得写回字幕、speech window 或审查结果。服务端只允许从该项目 `store/assets.json` 与 ingest roots 解析素材，使用工作区 ffmpeg 转码并缓存到项目 `.tmp/chronology/speech-review-audio/`；客户端不传文件路径，缓存不改变任何分析或审查事实。
- 确定性简体中文正字归一和 `config/transcript-normalization.json` 的精确替换规则可以在准备审查时写入 candidate spans，同时保留原文、命中规则和修正审计。精确替换采用一次、不级联的最长优先匹配；同一句在归一后仍作为 Agent 输入，不会因自动修正而跳过其他错字审查。其余 suggestion 初始 `accepted=true`，但只代表 UI 默认选择；用户改成不接受后，最终提交保留 baseline。字幕人工听音项以 `unresolved / completed` 表达审查状态，UI 只显示单个 `审查完成` Radio，结果由可编辑 `finalText` 决定。旧 `accepted / rejected` 字幕听音草稿在读取或保存时折叠到完成状态，rejected 的最终文本取原文。
- 窗口裁切只引用原有 segment id/范围；取消口播清除该 speech candidate 的 transcript truth 和 `有口播语音` tag，但必须保留重叠 visual span。缺少合法视觉事实时取消建议无效。
- 服务端批量提交是唯一正式写入口：它重新加载 baseline spans，校验 inputs/effective-glossary/normalization/trip hashes、segment 覆盖、建议边界、重复引用和视觉保留，然后一次写入 review、spans 与 meta。任一失败均保持原文件与 `pending-speech-review`。
- `PUT /api/projects/:projectId/speech-transcript-review/draft` 只更新审查 artifact 中的选择、文本和裁切范围，并重写同源 Markdown；它必须复核 baseline/effective-glossary/normalization/trip hashes，不得修改 spans。Console 对用户修改做短延迟自动保存并显示持久化状态，刷新后从 artifact 恢复。
- effective glossary 由只读内置 `resources/transcript-glossaries/*.json` 与工作区 `config/transcript-glossary.json` 合并，同名时工作区语境覆盖内置语境。摄影/视频稳定术语进入内置领域表，人名、地名、菜名、设备型号和个人用语进入工作区表。glossary v3 条目固定为 `canonical + context`；v1/v2 只兼容读取，下一次保存迁移到 v3。两层词表只供 post-ASR Agent 判断，不进入 ASR prompt。
...
## 0.17 2026-09-04 ASR 后置 Agent 字幕校对（已由 0.19 的统一审查合同取代）

本节的旧 Review Queue / pronunciation / Agent 直接落盘合同已由 0.19 的“口播与字幕统一审查”取代。保留的边界是：不新增 Analyze job、不重新执行 ASR，原始 report、segment 时间和分段不可变；所有非正字归一变更都先进入统一审查产物，再由用户整轮提交。
...
- `/chronology` 的用户路径由当前产物和 Supervisor live job 派生为六段横向阶段链：`span-rebuild -> 口播与字幕审查 -> chronology-build -> Agent 事件归并 -> Chronology 审查确认 -> Edit Flow`。每个阶段模块独立展示状态、数量/阻塞原因和自己的启动、重试、Agent 或审查动作；`Agent 事件归并` 必须显示可复制到 Codex Agent 的指令，完成前 Chronology 的人工编辑、合并、拆分与确认全部禁用。只有当前可执行阶段突出主按钮。空间刷新、重复重建等保留为维护操作；已恢复的历史 warning、inputs hash 与 checkpoint 属于折叠诊断，不得让已经完成的阶段看起来仍处于失败态。
- material-pattern output contract 采用严格七槽完整性：`store/spans.json` 任一行不是 7 个非空标签时，Supervisor project config 报告 integrity warning，`assertFreshSpans()` 阻止 chronology/downstream 消费。`span-rebuild args.mode=repair-patterns` 仅对当前不完整行调用同一七槽 LM 校验/纠错链，原子写回 patterns 并保留其它 span 字段与 review 结果。
- `span-rebuild`：现在是 span-builder。它只读取 `store/assets.json` 与 `analysis/asset-reports/*.json`，先确定性切片并校验所有 keep 的非音频素材都有 report、所有 material span 都有 Analyze 产出的 `visualObservation`；历史 recognized `speech-window` fine-scan window 若缺 `semanticKind`，必须从匹配 `interestingWindows` 或重叠 transcript 恢复 speech truth。新 Analyze 在窗口扩展完成、fine-scan/VLM 开始之前，对同素材 speech window 集合与每个 visual window 的 edit range 计算区间集合 IoU；仅当 `IoU >= 0.5` 时，从 visual window 扣除 speech 区间，删除不足 `15000ms` 的连续 remainder，把达到 `15000ms` 的 remainder 裁成独立 visual 候选，再交给后续素材召回决定是否采用。`IoU < 0.5` 的 visual window 保持原样；旧 report 的既有重叠窗口仍按历史事实读取，`span-rebuild` 和 speech review 不得事后切碎或改写它们。speech 只承载裁剪口播 truth，visual 不得继承 transcript；相邻 speech/mixed 候选按同素材、同口播通道、间隔 `<=3000ms`、合并后 `<=45s` 且可见中文 transcript `<=160` 字合并；行车 visual fallback/fine-scan 窗口按 passage 降碎片，默认同素材相邻 gap `<=60s`、单段 `<=90s`。通过本地 qwen 文本 LM 从每个 span 的 `type / semanticKind / transcript / visualObservation` 按素材时间顺序 8 个 span 一批生成 provisional 中文 `materialPatterns[]`；LM prompt 请求按输入顺序排列的 7-tag 短语行，代码按 chunk 顺序写回 span，并严格校验前四个槽；第 1 项是素材自身可观察的拍摄视角/构图形态，不得重复 `type` 的照片/视频载体语义，也不得写“建场/记录/成果”等后续剪辑用途；任一必需槽缺失或冲突都进入 failed span 列表，主 chunk 重试后逐项自动纠错最多三次，每次更新机器校验反馈并写 partial checkpoint，全部用尽才阻塞。纠错 prompt 是固定七槽 schema 加当前 item 与校验结果，不追加素材案例或错误类型特例；代码不做启发式改写或兼容映射。已完成 checkpoint 和 failed span 列表写到 `.tmp/chronology/span-rebuild.partial.json`。若存在 speech/mixed candidate spans，收口后写 `store/spans.json` 与 `store/spans.meta.json(status=pending-speech-review)`，并写 `.tmp/chronology/speech-window-agent-handoff.md` 交给 Codex Agent 生成统一口播与字幕审查产物；建议默认接受、需听音项保持未决，用户整轮提交后才可写最终 fresh spans；无 speech/mixed candidates 时可直接写 `status=fresh`。历史 speech-boundary 包络分析只保留诊断用途，可把明细写入 `analysis/speech-boundaries/<assetId>.json`，但不得覆盖 Qwen aligned timing；`effectiveSpeechStartMs/effectiveSpeechEndMs` 只作 timed transcriptSegments 缺失时的旧数据兼容。
- `chronology-build`：要求 spans 存在且 `status=fresh`，`pending-speech-review` 必须先由 Agent speech-window review 完成；再从 corrected assets + fresh spans + Pharos context + 项目 GPX/derived track 生成确定性 Chronology V2 候选 `media/chronology.json`；生成时先归属 Pharos 单点 actual window，再按结构化素材类型、时间连续性和 GPS 轨迹连续性聚合剩余 spans。写入候选后同时生成 `.tmp/chronology/event-consolidation-agent-handoff.md` 与 pending state
- `chronology-event-consolidation`：Codex Agent 读取候选 chronology、fresh spans 与 handoff，只返回需要合并的连续 ID 组、摘要和理由。普通模式要求所有来源都是相邻且地点一致的 `pending event`，可撰写合并后的中文标题。Pharos 吸收模式要求 `anchorEventId` 指向组内唯一的 confirmed `event-pharos-*`，其余来源只能是紧邻的普通 `pending event`；程序保留锚点 id、标题、地点和 confirmed 状态，允许周边 GPS 地点字符串与 Pharos 标签不同，只把时间扩成来源并集、按原顺序吸收 span 并采用 Agent 汇总摘要。日期零点不是边界；`route`、`gap` 和另一个 Pharos event 不可跨越。程序验证 inputs/candidate hash、完整相邻性、锚点唯一性、组间不重叠、span 顺序与无遗漏/重复，并将每次审计写入带 candidate hash 的独立 `.tmp/chronology/event-consolidation-audit-<inputs>-<candidate>.json`。只有 state 为 `completed/not-required` 才开放人工 Chronology 审查
- Chronology V2 事件/路线 `spanIds` 使用粗剪友好的展示/落片顺序：非照片 span 保持事件内顺序，照片统一追加到事件末尾；事件 `startAt/endAt` 仍按全部 rows 的真实时间范围计算，不因照片后置而改写时间事实。
- 新生成的 span 是素材片段索引，不承载时空解释、Pharos 引用、GPS evidence、route role、chronology event 或速度策略
- `store/spans.meta.json` 固定记录 `schemaVersion / status / generatedAt / inputsHash / assetCount / reportCount / spanCount / warnings`，并可记录 `speechReview.status / candidateCount / handoffPath / updatedAt`；`status` 至少包含 `fresh / stale / pending-speech-review`
...
- `store/assets.json` 的 `asset.capturedAt` 是项目内已修正时间；root `clockOffsetMs` 只在 Ingest / 迁移写入 asset 时应用，下游不得再次叠加。`media/chronology.json.assetIndex[].sortCapturedAt` 是等于 `asset.capturedAt` 的兼容索引。
...
- `/edit` 是 Chronology 后的正式用户入口，但只负责 Edit Unit 初始化和只读产物审查；新工作不再通过 `/script`、`script` job、Supervisor `edit-flow` job 或固定 Timeline 阶段推进
- `editRuleCategory` 是 Edit Flow 结构输入；它独立于可选 `styleCategory`
- `edits/<editId>/config/edit-unit.json` 是一个剪辑单元的正式初始化记录，保存 `editId / editRuleCategory / styleCategory`
- `edits/<editId>/planning/flow-plan.json` 是 Codex Agent 基于 raw edit rule、项目上下文和 capability registry 生成并维护的显式执行计划；当前计划必须写 `plannerPolicyVersion=codex-agent-v1`，hash stale 或 policy stale 时，任何 capability step 都不得信任
- Flow Plan、planning document、material recall 和 Agent/SubAgent step 不再由 Console 或 Supervisor 外部执行器生成；代码只提供只读展示、schema/合同校验和确定性 Resolve/Timeline 辅助能力
- capability registry 是可执行原子库，不是固定阶段链；每个 capability 声明输入、输出、默认 runner、gate 与状态，执行记录写入轻量 `edits/<editId>/runs/current.json`。run record 只保存摘要、输出路径与审查状态，不得内联完整 KTEP、timeline clip 列表、Resolve clip 明细或 `hostSummary.clips`
...
- Codex Agent 必须把剪辑规则自由正文中对风格层的使用意图结构化写入 `flow-plan.json.styleUsage`；代码只读取 `styleUsage`，不得关键词解析 edit-rule markdown
- 剪辑规则自由正文也可以用自然语言要求 `SubAgent`、分片粒度或 capability-specific 人工规则；Codex Agent 必须把执行方式转译为 `step.execution`，把适用于下游的人工规则写入对应 step `notes`，runtime 只读取 confirmed Flow Plan，不直接解析 markdown
- Agent/SubAgent packet 必须注入当前 capability step context / notes；confirmed step notes 高于 skill 通用口径和 agent 启发式，但不能覆盖系统/协议安全合同
- Codex Agent 必须选择剪辑规则显式要求或后续执行必需的最小 capability 集合；`config/edit-rules/*.md` 是用户维护的规则源，Agent 不得为适配 Flow Plan、skill、schema 或测试自动改写规则内容；规则与系统合同冲突时只能标记产物 stale、阻塞或请求用户确认规则变更；`trip.event_table` / `material.archive` 只有在规则明确要求独立事件表、行程表、素材档案、素材库文档或单独人工审查产物时才出现
...
- `script.generate` 只有在剪辑规则明确需要前置文本稿 / beat 稿时才出现；`material.recall` 只输出 `material-slots.json`，不再生成或消费 `segment-plan.json`，并应把同一 chronology event 内照片作为尾部照片包而非插入非照片素材之间；`resolve.media_sync` 先把事件素材同步到达芬奇 Media Pool；`timeline.generate` 是 deterministic Resolve rough-cut 创建步骤，读取 `edit-framework.md + material-slots.json + spans + assets + chronology`，只从已同步 Media Pool 选用素材，不再要求 `script/current.json`，同一事件内落片时保持非照片在前、照片尾部打包；单遍落片回读实际帧位后生成 `.tmp/edit-flow/<editId>/timeline/current.srt`
...
- 新 Edit Flow 不做旧 `script/`、`timeline/`、`subtitles/` 根路径兼容迁移；`timeline.generate` 的正式输出是 Resolve timeline，`.tmp/edit-flow/<editId>/timeline/current.json` 只作为本机临时 KTEP/manifest 审计，`.tmp/edit-flow/<editId>/timeline/current.srt` 是源语音字幕伴生产物；所有自动生成字幕、SRT/VTT、字幕审查稿和配音插件合并文本共享行末不补 / 不保留 `。` 或 `.` 的规则，问号、叹号和句内标点保留
...
- `/edit` Resolve 工程维护必须先通过 workspace 级 Resolve assets installer 统一安装/更新 Resolve 侧插件与效果库，再执行依赖 Resolve 环境的维护动作。installer 使用 `config/resolve-assets/manifest.json` 作为唯一清单，当前覆盖旁白配音插件与 Fusion Edit Effect 模板；它只把正式资产复制/校验到本机 Resolve 用户目录，不写 Flow Plan run record，不修改时间线。旁白插件的 `Scripts/Edit` 目录只允许安装 `KairosVolcVoiceover.lua` 这一项，工作区链接写入 `Fusion/Config/KairosVolcVoiceover/kairos_workspace.json`，并清理 user/system 两边历史残留的同名脚本和 `KairosVolcVoiceoverLib` 菜单目录。`重链素材路径` 按钮必须强制先运行 installer，installer 失败时阻塞 relink；成功时 relink 摘要必须带上安装/校验结果，UI 也必须提供独立的安装/刷新入口和状态展示。
...
- 插件目录固定为 `apps/resolve-volc-voiceover-plugin/`，安装到 Resolve `Fusion/Scripts/Edit` 菜单；它是独立 Resolve Lua/Fusion UI 脚本，通过本机 `127.0.0.1:8940` Supervisor TSV API 请求火山合成音频，若 Resolve Lua 缺少 socket/TCP 能力则自动改用 Supervisor 管理的 workspace 文件 IPC（`.tmp/resolve-volc-voiceover-plugin/ipc/requests|responses`），Lua 自己完成 Resolve Media Pool 导入、音轨选择、timeline append、marker 与保存。插件不再依赖 Python backend、`.cmd`、curl 或代理链；HTTP 与文件 IPC 都必须保持本地、command-free。
- 插件负责枚举当前 timeline 的 subtitle tracks，并要求用户在插件面板里指定旁白字幕轨道；字幕列表只加载该轨条目，列表支持鼠标多选 / 范围选择 / 追加选择，播放头定位和 Resolve In/Out 批量选择也只作用于所选轨道。多字幕轨时间线不得默认全轨混扫，避免把 source-speech / 原声字幕误当旁白合成。官方 Resolve scripting API 当前没有稳定 `GetSelectedTimelineItems()`，所以第一版不承诺读取 Edit 页原生多选字幕。
...
- 单选字幕时生成一个音频文件和一个 timeline audio item；多选字幕时不做启发式拆分，直接把用户选中的所有字幕按 timeline 时间排序、移除每行末尾 `。` / `.`、拼成一个 TTS 文本、生成一个 merged audio item，并插入到第一条所选字幕的起始帧。merged unit 的 debug record 必须记录 `isMergedGroup / groupSize / sourceSubtitleIds / sourceSubtitles`，便于排查和后续 replacement。Supervisor 负责项目匹配、`voiceoverMedia` 候选选择、runtime profile 读取、火山 direct fetch、cache、音频落盘、duration probe 与 manifest 写入，但不连接 Resolve。生成文件必须落在当前 Kairos 项目 `config/project-brief.json.voiceoverMedia` 声明的配音媒体根下。`voiceoverMedia.path` 是主路径，`voiceoverMedia.alternatePaths[].path` 是多设备候选路径，`voiceoverMedia.resolveProjectAliases[]` 可显式把临时 / debug Resolve 工程名归属到该 Kairos 项目；Supervisor 按当前设备上第一个可写候选路径创建 `<safe Resolve project>/<safe timeline>/` 正式媒体目录，目录中只保存 Resolve 直接导入的 `vo_<unit>_<request>.mp3`。请求缓存、debug manifest 和插件日志必须写到匹配项目的 `projects/<projectId>/.tmp/resolve-volc-voiceover-plugin/` 下；只有无法匹配项目的早期 bootstrap 日志可退到 workspace `.tmp/resolve-volc-voiceover-plugin/`。若当前 Resolve 工程名无法唯一匹配 Kairos 项目，或项目未配置可写 `voiceoverMedia`，合成必须阻塞并提示配置，不再把正式音频产物写到 `~/Movies/KairosVoiceover/`。
...
- `execute_root` 的正式 render 合同已经收紧为：
  ...
  - 每个 `AddRenderJob()` 后必须用 `GetRenderJobList()` 校验 `OutputFilename` 是本批 Source Name；`OutputFilename` 是 Resolve 队列 UI 摘要，需接受英文 `and more` / `and N more` 与中文本地化 `及更多` 这类 source-name 多片段摘要；不匹配时删除该 job 并在 `StartRendering()` 前失败，不能等渲染后 validation 才发现
...
1. `coarse-first analyze` 已把 ASR 纳入视频细扫前链路
   ...
   - 工作区 `config/runtime.json.asr.backend` 显式选择 `qwen3 | whisper`，选择结果是严格运行路由而不是偏好；配置层不再保存模型路径、语言或分片长度：
     ...
     - 识别阶段不注入领域词表、工作区个性词表、文字归一或行程热词：不得向 Qwen 传 `context`，也不得向 Whisper 传 `hotwords / initial_prompt`。`resources/transcript-glossaries/*.json` 与 `config/transcript-glossary.json` 合并后只供后置 Agent 结合完整句子、相邻口播和当前时间段行程裁决；`config/transcript-normalization.json` 仅在 post-ASR 候选字幕准备时执行
     - ASR backend 必须返回带时间的 aligned token；无论底层 Qwen3 + ForcedAligner 或 Whisper 返回字符还是词，TS 写 checkpoint 前都统一展开为 Unicode 单字符 token，并只在原 token 时间范围内单调分配字符时间
     - TS 在对 aligned token 做自适应长时异常修复后，按 ASR 原始标点、36 字逗号均衡拆分和 `<1500ms` 短间隔合并确定性生成正式 segments；不调用文本模型，正文与时间从连续原 token range 重建
   ...
   - `drive` 的 `speech` 和 `visual` windows 已正式分语义，并通过 `semanticKind` 继续向后传递；扩窗后、fine-scan 前按 edit range 集合计算 IoU，`IoU >= 0.5` 时从 visual 扣除 speech，连续 remainder `<15000ms` 删除，`>=15000ms` 裁成新的 visual 候选，最终保留权仍属于素材召回
   - Analyze speech window generation is deterministic and subtitle-independent: it reads repaired aligned-token timing before transcript segmentation, derives an asset-local log-gap upper-tail boundary, applies a token-duration cadence guard plus singleton/sparse-tail checks, and never uses a global seconds threshold, subtitle sentence boundary, text length, or maximum window duration. The resulting speech windows remain inputs to unified finalize, fine-scan, and recall.
   - `/chronology` 的 `span-rebuild` 对 speech/mixed 使用同 asset、同口播通道、间隔 `<=3000ms`、合并后 `<=45s` 且 transcript `<=160` 字的合并规则；新 Analyze 已在 fine-scan 前按 `IoU >= 0.5 + 连续 remainder 15000ms` 规则处理 speech/visual 重叠，旧 report 的既有重叠保持只读兼容，行车 visual 继续按 passage 降碎片；细粒度停顿继续由 `transcriptSegments` 表达。speech window core 精确取组内首末 aligned token，edit range 只在两侧各加一次 `250ms` 且不吸附镜头切点；历史 speech-boundary 包络结果只作诊断/无 timed transcriptSegments 时的兼容字段
   - 素材身份协议为 `materialIdPolicyVersion=human-source-v1`：每个 root 声明稳定 `rootCode`，Ingest 生成短 source locator，例如 `C0506_zve1_day1`；Analyze / span-rebuild 生成 `<assetId>_<type>[_<semanticKind>]_s<floorIn>-<ceilOut>`，例如 `C0506_zve1_day1_drive_speech_s0-7`；照片 span 使用 `<assetId>_photo`。这些 id 是正式外键、报告文件名和审查引用，不再使用随机 UUID 或 `asset__` / `span__` namespace 作为新素材/片段主键。
2. Edit Flow 的素材召回与 Resolve rough-cut 生成已消费 transcript 证据
   - transcript 不再只是附属说明，而是候选召回和 beat 写作的正式输入
   - `material-slots.json.treatments` 是召回阶段向粗剪传递音频/速度建议的唯一正式覆盖字段；缺少 `treatments[spanId]`、`audio` 或 `speed` 时按默认 `{audio:0,speed:1}` 读取，因此默认有声正常速 span 不写 entry；被选中的非照片、非航拍 span 只要有 transcript、transcriptSegments、`semanticKind=speech/mixed` 或 `materialPatterns=有口播语音` 就不得静音，保护覆盖 `drive / broll / timelapse / talking-head`；航拍视频原音在粗剪中一律禁用，即使 ASR 误识别出 speech truth 也不进入原声保护；未选入的 protected speech-backed 非照片 span 由 `coverageAudit` 暴露，代码不得盲目追加到主粗剪时间线；无口播视觉 span 需要静音时显式写 `audio=-100`，只有 `drive / aerial` 可显式 `speed>1`，且 `speed` 必须是 `1..5` 的整数倍
   - 如果某拍最终保留原声，material.recall 按稀疏 treatment 省略默认 `audio=0`；timeline.generate 按 recalled chosen spans 顺序落位，不再依赖 script beat 的 merged audio units 作为粗剪前置
   - `source-speech` 当前以过滤后的口语 transcript 为真值：导航播报、录制口令和设备提示不应进入 merged audio units，也不应成为 source-speech 字幕
   - Analyze 当前产出的 `transcriptSegments` 已经通过 token 连续覆盖、字级时间异常修复和确定性 punctuation-gap 拆句校验；字幕文字校对必须保留这些分段与时间
3. 风格分析与 Edit Flow 的交接语义已经收口
   ...
   - 这些信息不会自动成为 capability 输入；剪辑规则自由正文必须先说明要使用哪些层，再由 Codex Agent 写入 confirmed `flow-plan.json.styleUsage`
  ...
  - 首轮 stage 调用默认应保持 lean context，只在 reviewer 要求返工时再把 previous draft 带回 writer
   ...
   - `material.recall` 的正式结构化输出只有 `material-slots.json`；`segment-plan.json` 不再是正式输出或下游输入；素材事实只消费 `edit-framework.md + store/spans.json + store/assets.json`，人工规则只消费 Flow Plan 当前 step context / notes
   - `resolve.media_sync` 是 deterministic runner，把 confirmed chronology 中的事件素材同步到达芬奇 Media Pool；chronology 在这里仅用于 Resolve Media Pool bin 组织和工程归档，Media Pool 是素材归档真相，不新增 JSON 归档层
   - `timeline.generate` 是 deterministic runner，按 `material-slots.json` 的 chosen spans 和顺序从已同步 Media Pool 素材创建 Resolve rough-cut timeline；chronology 只用于 Resolve path/bin/context 映射，不得重排或改变选择；`.tmp/edit-flow/<editId>/timeline/current.json` 只是本机临时 KTEP/manifest 审计文件，不是 Resolve 不可用、素材缺失或 source range 校验失败时的成功兜底
...
4. 字幕已支持双路径
   ...
   - 时间线展示层若因版式需要把单条 semantic transcript cue 拆成多行，只能视为渲染排版；不得把新的句界或重算时间写回 transcript review、asset report 或 audio checkpoint
5. GPS 链路已形成当前最小闭环
  ...
  - 项目内跨设备时钟漂移当前正式通过 ingest root 统一修正，而不是继续让 timeline 末端猜：
    - `config/project-brief.json` 对应 mapping 的 `clockOffsetMs?` 表示项目内该 root 的统一时钟偏移，但只在 Ingest / 迁移写入 asset 时应用
    - 单素材 `captureTimeOverrides` 继续存在，并直接写成最终 `asset.capturedAt`，不再额外叠加 root offset
    - `store/assets.json` 的 `asset.capturedAt` 当前是唯一正式素材时间真值；原始解析时间仅保留为 `rawCapturedAt` / metadata 审计字段
    - `media/chronology.json` 的 `assetIndex[].sortCapturedAt` 是兼容镜像，生成时等于对应 `asset.capturedAt`
...
- source-speech audio units 默认按 `<= 3000ms` gap 合并，并在合法范围内保留 `120ms` / `180ms` breathing；Resolve rough cut 的 source-speech clip 优先以有效 `transcriptSegments` 的首末时间作为 Qwen aligned 核心窗口，再在外侧只加一次 `250ms` / `250ms` padding。timed segments 缺失时才依次兼容旧 `effectiveSpeechStartMs/effectiveSpeechEndMs` 与 span source/edit window，响度边界不得覆盖对齐结果
- Resolve rough cut 会写入正式 clip color 批处理标记：照片 video item 为 `Blue`，延时 video item 为 `Purple`，行车 video item 为 `Brown`，航拍 video item 为 `Teal`；没有这些视觉分类颜色的普通有声 video item 为 `Orange`，剩余可听 linked audio item(s) 为 `Orange`。航拍 linked audio 在粗剪中强制 disabled，因此不参与可听音频归一化筛选。这样照片/延时/行车/航拍可用于视觉批处理，音频仍可按 `Orange` 归一化。粗剪生成还会创建/复用 Resolve Color Groups `Kairos Photos` 与 `Kairos Timelapse`，把照片/延时 video item 分别分组，供 Color 页 `Group Post-Clip` 批量效果使用。不能用 Color-page filter 或 flag 代替 clip color，因为 Color 页选择不会同步成 Edit 页 timeline selection，Resolve flag 也会沿 linked items 扩散，无法精确表达这些批处理集合。
...
- 如果同一 `asset` 同时产出 source-speech 与 silent `drive / aerial`，时间线应先锁定 source-speech 的 source ranges，再把 silent montage 剪成非重叠 remainder；同一 source window 不得同时出现在两条路径里
  - 如果同一 `drive / aerial asset` 被多个 silent montage beat 重复引用，placement 还应继续扣掉前面已消费的 source ranges；后续 beat 只能使用新的 remainder
     - photo-only beat 当前落成静默镜头；除非剪辑规则 / confirmed Flow Plan 或 `timelineStillDurationMs` 显式覆盖，照片默认静帧时长为 `1000ms`，且默认不生成字幕
- Timeline 的 chronology guard、beat 排序与 selection 排序当前都必须统一消费已修正的 `asset.capturedAt`；Chronology V2 的 `assetIndex[].sortCapturedAt` 只作为兼容索引，不得触发二次 offset 修正
9. Analyze 恢复与资源口径已经补到项目级正式设计
   - coarse prepared state 会写入 `analysis/prepared-assets/<assetId>.json`，只保存 finalize 之前的准备输入；checkpoint 同时记录源文件 size/mtime fingerprint，恢复时必须校验 fingerprint、非空代表帧和帧文件存在性/新鲜度，任一不满足都重新粗扫；源文件更新后已有 image proxy/keyframes 不得继续复用
   - 照片代理提取失败属于 coarse-scan 失败：必须透传媒体工具的可诊断错误，不写 `sampleFrames=[]` checkpoint，也不能延迟到 photo finalize 后误报为 VLM 描述为空
...
当前实现已经开始把旧的 `slice + 五轴语义 + 单阶段 arrangement` 迁到 Codex Agent 驱动的剪辑准备链：
...
- `materialPatterns` 是 span-builder 阶段由 LM 从最小 span 文本事实生成的中文脚本可消费短语数组，不再是 `{ phrase, confidence, evidenceRefs }`；生成上下文只允许 `type / semanticKind / transcript / visualObservation`，不传 span id、`labels`、`report.summary`、GPS / 时间 / Pharos 归属；`span-rebuild` 当前按素材时间顺序 8 个 span 一批调用本地 qwen 文本 LM，prompt 要求 `string[][]` ordered rows 且每行正好 7 项，代码按 chunk 顺序拼回 candidate spans，并严格校验前四个槽：受控 `拍摄视角/构图形态`、提取 `当前环境`、可观察自然 `天气光线`、provisional 二元 `口播语音`；第 1 项不得重复 photo/video 等载体类型或写后续剪辑用途，slot1/slot2/slot3 是否被画面事实支持由 LM prompt/rubric 约束，代码只用结构字段和错槽保留词拦截硬冲突（如 `photo + 行车/运动/口播视角`、`drive + 航拍视角`、`无口播语音 + 口播视角`、视角/口播 tag 出现在环境或天气槽），不得用关键词正则二次判定支持性；第 5 项是 LM 情景故事或 `情景不明`，第 6-7 项必须来自 LM；代码不得启发式补写、替换旧词或兼容迁移。允许的稳健性修复只限协议层：解析本地 LM 输出时可提取第一段完整 JSON 或补齐明显缺失的 JSON 外壳，校验失败时记录具体 slot issue，并在第二次 chunk 尝试与后续单条自动纠错时把失败原因、期望口播槽值和 expected row count 反馈给 LM。单条纠错 prompt 只保留当前 item、机器 validation feedback 与固定七槽 schema，不按八标签、重复标签、截断等错误类型累积示例或特例指令。主 chunk 仍失败后，每个失败 span 最多自动纠错三次；每次调用后更新 progress 和 partial checkpoint，次数用尽仍不合格才让本次 rebuild 失败。代码仍不得根据 `visualObservation` 自行创造环境、故事或 free tags；必须在 `.tmp/chronology/progress.json` 写入切片、chunk LM、failed-list、ETA、当前 attempt、重试、warning 与最终写入进度，并在 `.tmp/chronology/span-rebuild.partial.json` 写入已完成 materialPatterns checkpoint 和 failed span 列表。`/chronology` 将 live job 与 durable terminal evidence 分开，失败或阻塞后只关联 `extra.jobId` 匹配的终态 progress，并持续展示错误、失败计数与 checkpoint 续跑入口；存在 speech/mixed candidate 时，`store/spans.json` 是 pending candidate truth，必须由后置 Codex/Agent SubAgents 完成 speech-window 裁切并写回 fresh spans 后才可被 chronology-build 消费
...
- Edit Flow 现在正式改为：
  ...
  3. `capability step -> declared inputRefs / outputRefs / gate / notes`
  4. `step run -> edits/<editId>/runs/current.json`
- `material.recall / script.generate / timeline.generate` 都只是可选 capability，不再构成固定阶段链。默认 `material.recall` 的素材事实只需要 `edit-framework.md + store/spans.json + store/assets.json`，人工规则来自 Flow Plan 当前 step context / notes，由 corrected `asset.capturedAt` 计算 span 真实时间，不默认读取 asset reports 或完整 chronology。
...
- `/ingest-gps` 除了单素材“素材时间校正”卡片外，当前还应并列提供 root 级设备时钟偏移 editor，用 `±HH:MM:SS` 输入并保存 `clockOffsetMs`；下一次 Ingest 会把该偏移写入 corrected `asset.capturedAt`
...
├── flow-planner.ts     # Codex Agent plan policy、freshness evaluator、只读加载与合同校验
└── index.ts
```
...
```
`/edit`
  → 初始化 `edits/<editId>/config/edit-unit.json`，选择 `config/edit-rules/*.md` 中的剪辑规则与可选 layered style profile
  → Codex Agent 读取 raw markdown、capability registry、project brief、fresh spans、confirmed chronology、Pharos context 与 analysis availability
  → 写入 `edits/<editId>/planning/flow-plan.json`，`plannerPolicyVersion=codex-agent-v1`
  → `/edit` 只读展示 Flow Plan 与 artifact 状态
  → Codex Agent 按 step 生成或执行 capability 产物
     - 解析 `inputRefs`
     - 注入当前 step context / notes 到 Agent/SubAgent packet
  ...
  → 带 `gate=human` 的 step 只在产物中暴露人工审查状态；UI 不提供确认或推进入口
```

补充口径：

- 正式剪辑流程以 `Pharos + Chronology V2 + 剪辑规则` 为主输入
- `trip.event_table` 是可选 chronology 事件组织能力，只读取 confirmed `media/chronology.json`；素材级 spans / assets 由声明它们的 `edit.framework`、`material.archive` 或 `material.recall` 精确消费
- `edit.framework` 直接读取素材级事实时也必须保持 handoff 合同：`全片章节` 只做宏观概览，`分段操作稿` 是唯一可执行 FW beat 边界，不输出 `beat 边界索引`；全文不写 chronology/event/route/gap/span/asset id，`spans` 列必须写类型数量与视频语音拆分；系统合同不规定固定的素材召回提示章节名
...
- `resolve.media_sync` 是 deterministic runner，只负责把 confirmed chronology 对应素材同步进达芬奇 Media Pool；chronology 在这里仅用于 Resolve bin 组织与工程归档，Media Pool 本身是素材归档真相。同步必须复用已有 MediaPoolItem：目标事件目录一致则跳过，目录变化则只移动到新事件目录而不重导入，同步结束后清理空事件目录；run record 只保留 imported / reused / moved / pruned 摘要。Resolve 侧的 bin name / item name / hostSummary display label 只允许作为人工识别和 debug 回声，不得回流为 Kairos 地点、路线、事件标题或字幕事实。
- `timeline.generate` 是 deterministic runner，只消费 `edit-framework.md + material-slots.json + store/spans.json + store/assets.json + confirmed media/chronology.json` 并按 `material-slots` 的选择和顺序从已同步 Media Pool 取素材；航拍视频 linked audio 一律强制 disabled；有声非航拍 speech/mixed 非照片 clip 在 Resolve append 前按 timed transcriptSegments 首末边界扩展一次 source padding（头 `250ms`、尾 `250ms`，clamp 到素材边界），chronology 只用于 Resolve path/bin/context 映射，不得硬性读取 `script/current.json` 或 `segment-plan.json`
...
- `.tmp/edit-flow/<editId>/timeline/current.json` 作为本机临时 KTEP/manifest 审计输出保留，`.tmp/edit-flow/<editId>/timeline/current.srt` 作为按 Resolve 实际帧位生成的源语音字幕保留，但 Resolve rough-cut timeline 才是用户可见成功标准
...
读取 Flow Plan 声明的前序产物
  ...
  → 校验 material-slots：按稀疏 treatments 解析默认 `{audio:0,speed:1}`、dropped asset 不进入召回、非 drive/aerial 不允许 speed>1、有 speech truth 的非照片非航拍 span 不允许 audio<=-100、航拍音轨由 timeline.generate 强制 disabled、照片必须显式静音
  ...
  → official Python host 先通过 `resolve.media_sync` 复用/移动/导入达芬奇 Media Pool 的项目全局 `Kairos Project Media` 素材：目标事件目录一致则复用跳过，目录变化则移动，缺失才导入，并在同步结束后清理空事件目录；随后在 `Kairos Timelines` 中通过 Resolve 原生 `MediaPool.AppendToTimeline` 创建/替换 Resolve rough-cut timeline，自动写入并回读校验照片 `Blue`、延时 `Purple`、行车 `Brown`、航拍 `Teal`、普通有声/剩余可听 linked audio `Orange` clip color 批处理标记，创建/复用 `Kairos Photos` / `Kairos Timelapse` Color Groups 并分配照片/延时 video item，同时回读校验 source range / still duration / timeline gap-free placement；航拍 linked audio 强制 disabled；有声非航拍 speech/mixed 非照片 clip 的回读 source range 包含基于 timed transcriptSegments 的默认 `250ms` head / `250ms` tail padding，照片默认校验 `1000ms`，只有剪辑规则 / confirmed Flow Plan 或 `timelineStillDurationMs` 可覆盖
  → 时间线构建先对旧 `spans/material-slots` 执行 Analyze overlap 策略的兼容过滤：同素材 selected speech union 与 visual 最终 source range 的 `IoU >= 0.5` 时扣除 speech，连续 remainder `<15000ms` 删除、`>=15000ms` 才落为裁切后的 visual clip；每个 selected span 的基础 `clip-xxxxx` 在过滤前分配，drop/split 不得推动后续锚点编号
  → 若显式选择 `resume-from-current-playhead`，host 在同一个既有 rough-cut timeline 中读取播放头，以命中的完整 video item 末帧和其 `clip-xxxxx` 名称作为续接锚点；只清除锚点后的 video/audio/subtitle，按确定性计划过滤并追加锚点后的 clips。失败期间保留临时 duplicate，全部追加、source range、still duration、颜色、分组和连续性校验通过并保存工程后才删除 duplicate；Node 层只为新追加后缀按实际绝对帧位生成 `remaining.srt`，不导入字幕、不改前缀
  → Resolve 单遍原生落片成功后，按 host 返回的 `actualStartFrame / actualEndFrame` 重定时 clip 并写 `.tmp/edit-flow/<editId>/timeline/current.srt`
  → Resolve timeline 成功后以 `latest-only` 保存项目级 `[Edit]` DRP；失败只写入 `drpSnapshotWarning`
  → 单遍落片和实际帧位校验通过后才写临时 `.tmp/edit-flow/<editId>/timeline/current.json` 审计
```

补充口径：

- `timeline.generate` 不再调用 LLM / Agent reviewer，不写 `rough-cut-base / segment-cuts / reviews / agent-pipeline` 作为正式门槛
- `material-slots.json.treatments` 是音量/加速唯一正式覆盖来源：缺省 `{audio:0,speed:1}`，默认字段可省略；`audio` 单位 dB，静音 `-100`；非照片非航拍 span 只要有 transcript、transcriptSegments 或 `semanticKind=speech/mixed` 就不得静音；航拍音轨总是在 timeline.generate 中强制 disabled；`speed` 单位倍速，但只能写 `1..5` 的整数倍
...
- source-speech SRT 的 cue 文本与素材内相对时间仍来自 transcriptSegments，但 timeline 绝对时间必须基于 Resolve 单遍落片后返回的实际 clip 帧位，不能基于预估毫秒 cursor；speech/mixed clip 以 timed transcriptSegments 为核心并只扩展一次前后各 `250ms` 的实际 Resolve clip 边界，不把字幕铺满前后静音余量；航拍 clip 因强制静音不进入 source-speech SRT。transcript segment 是上游时间单元，SRT 输出必须重新校验正式 36 文本单位句界：只有 `，` / `,` 是软标点，其余 Unicode 标点一律结束句子；超长句通过动态规划选择满足上限所需的最少逗号边界，并在该段数下最小化各段相对平均长度的偏差，只有单个无可用逗号的 clause 仍超限时才允许字符兜底。同一 Resolve clip 内相邻候选在映射 gap `<1500ms` 且合并正文 `<=36` 时继续合并；一个最终 cue 若以软逗号结尾，必须把该逗号提升为 `。`，source-speech SRT 导出不得再删除这个句界。拆分后的 cue 在原 segment 时间内按文本单位比例连续分配，不能修改 span 文本和 segment 时间。`timeline.generate` 在落片完成后生成严格对齐的 SRT，不为字幕再次重建视频时间线，也不把 SRT 导入列为已执行能力。
...
- `postlock.subtitle_narration` 的旁白框架阶段必须先从当前 Resolve timeline 导出 clip-level packet：`.tmp/edit-flow/<editId>/postlock/current-timeline-clip-packet.json`。Resolve timeline 在此阶段只提供锁定 clip 顺序、轨道、timeline/source range、MediaPool source 和 `clipId / assetId / spanId / eventId` 映射；Resolve item name、MediaPool folder name、rough-cut `hostSummary.clips[].eventTitle`、`eventTitleFromResolveName()` 解析结果只允许是 debug/display-only，不得作为地点、路线、事件标题、地貌、水文、植被或字幕写作事实。该阶段以字幕轨覆盖判定口播 clip，并使用剪辑规则附录的 Markdown pack-list v2 作为正式正文格式：顶层是 `口播 pack / 行车 pack / 航拍 pack / 照片序列 / 延时 / 延时序列 / 普通视觉` 等叙事单元，`整体` 或 `摘要` 写 pack 级理解，`clips` 保留 leaf clip 描述。行车 / 开车无口播 clip 可以在正文中按相邻连续关系形成 pack，航拍 clip 可以按相邻相关关系形成 pack，照片序列和同编年史事件的延时序列可以合并为一个顶层条目；但 clip-map 只作为轻量 v2 边界索引，`entries[]` 只存 `marker + clips` 并覆盖所有 Resolve video clip，`packs[]` 只存 `title + entries`，不得复制 packet 中可反查的 `assetIds / spanIds / previousClipIds / summary` 等事实。普通无字幕非照片 clip 在 leaf 层只能归属一次，pack 只作为写作组织和后续字幕审查上下文。无字幕视觉事实只来自 `visualObservation`，不能读取 `materialPatterns` 生成、分类或兜底视觉描述；当当前 span 是 `semanticKind=speech/mixed` 但 Resolve clip 没字幕时，生成器按当前 item 的 `sourceStartFrame/sourceEndFrame` 和素材 fps 换算 source range，优先使用同 asset 重叠 visual span 的 `visualObservation`，无重叠时只允许使用 `<=15s` 最近 visual span，仍无匹配才回退当前 speech/mixed span 自带 `visualObservation`。无字幕视觉 / 航拍 / 延时正文必须贴近剪辑规则附录，用场景 / 动作短句交代路线、事件和画面推进关系，不得把 `visualObservation` 压缩成 `雨后湿滑、高速路面、道路延伸、车流穿行` 这类逗号标签串。口播 pack 必须基于当前字幕内容写短摘要，不能退化成地点 / 事件 + `口播`，不能粘贴字幕全文，也不能泄漏 `口播信息待人工复核` / `待人工复核` 这类内部占位。口播 clip 只有相邻、时间连续且字幕表达连续或同一话题时才可合并；允许合并时须在 packet 中记录显式 merge group，跨事件边界不能只因字幕片段重叠就合并。正式产物必须同时写 `edits/<editId>/postlock/narration-framework.md` 与 `edits/<editId>/postlock/narration-framework.clip-map.json`，并通过 `node scripts/validate-postlock-narration-framework.mjs projects/<projectId> <editId>` 后才允许 run record 进入成功或 awaiting-review 状态。
- 旁白框架正文的定位括号属于创作上下文，不是 GPS 审计日志：只能写可读地名 / 道路 / 村寨锚点以及山体、江河湖泊、植被、桥隧和地貌特征；`GPX±Ns`、`机内GPS`、经纬度、GPS 来源、匹配误差、source path、confidence、`�` 和 mojibake 显示串必须留在 packet / 审计 JSON 之外或被丢弃。定位事实只能来自 `media/chronology.json`、GPS/GPX、reverse-geocode cache、precise geo audit 与当前 asset/span，不得从 Resolve item name 或历史 rough-cut `hostSummary.eventTitle` 反推。行车 leaf 按当前 Resolve clip 的源时间中点取定位，不能把 pack 标题或整段 `A → B` 路线复制到每个子 clip；航拍可以用 confirmed chronology event 组织 pack，但 leaf 定位仍要回到实际飞行位置和视域。道路、桥梁、隧道、互通、收费站、跨江 / 跨海通道等线性交通设施属于有效可读锚点；项目部、管理中心、停车场、观赏点等 POI 全名只能留在审计数据，正文最多抽取其中真实交通设施名。定位括号还必须按地貌 / 水文 / 植被类别去重：已写出具体河湖海、峡谷崖壁、山口峰丛、桥隧道路或植被带时，不得追加 `江河湖泊 / 峡谷崖壁 / 林带植被` 等泛类标签；`林带植被` 不得进入正式正文，证据充足时应改写为具体植被，证据不足时省略。行车 / 航拍 pack 不得跨 confirmed chronology event 强行合并，`整体` 必须归纳子 clip 的地貌、水文、道路、植被、村镇和天气路况变化。
- `postlock.subtitle_narration` 的最终字幕审查稿 `edits/<editId>/postlock/subtitle-review.md` 必须在 `narration-framework.md` 人工确认后生成；表格列名、行粒度、跳过规则、占位文本和单元格内 clip 范围标注方式由当前剪辑规则与 confirmed Flow Plan step notes 定义，全局架构不固定项目级表头。审查稿必须参考 Flow Plan 授权的 `styleUsage.literary` 风格档案，并参考当前 Resolve timeline 每个 clip 或 clip 序列组的真实时长；自动生成或修订的字幕列不自动补 `。` / `.`，最终 SRT 也必须使用同一清理规则。连续相邻的行车或航拍视频可以先聚类判断整体动机，再按剪辑规则要求的粒度编写，首尾 CLIP 可承上启下并参考前后口播但不得覆盖已有口播字幕。普通非照片 clip 默认一条短 caption；只有延时摄影和延时序列允许突破一 clip 一句，且字幕数量不设固定 2-3 条上限，应优先保证画面叙述完整，按天光、云雾、色温、车流 / 人流、山体显露、夜色或晨雾回升等可观察变化拆成足够清楚的多条短字幕；同一延时 clip 拆多条时必须保留 clip 标签 / 范围或子范围，最终 SRT 按真实 timeline 时间分配。普通行车、航拍和视觉 clip 不因“较长”自动获得拆分例外。照片和照片序列默认不生成旁白字幕；若当前剪辑规则要求保留对应行，按剪辑规则指定的占位文本填写。被聚类且标记为 `开车` 的片段必须结合位置、路线 / 道路名推断与前置视觉分析景观事实来写，无法确定时按剪辑规则指定的方式标注待确认，不得编造。
- `postlock.subtitle_narration` 的字幕审查稿重写若用户在当前任务中提供或明确认可旁白样本，应先读取该样本并抽取其写作机制，作为本轮修稿参考；样本可来自 Resolve timeline / intro / 已成片字幕，也可来自用户粘贴文本、手稿或其它旁白材料。普通素材参考样本只是当前任务的风格修正手段，不自动沉淀为后续项目的永久写作规则。字幕列应写成贴时间线的 caption，而不是素材说明或地理审计摘要：普通非照片 clip 默认一条短句，只有当前剪辑规则、confirmed Flow Plan notes 或用户明确允许时才合并相邻普通 clip，且不得超过允许的合并宽度；只有延时摄影 / 延时序列必须按画面内部时间变化、天气天光、云雾推进、交通流动、山体显露、夜色或晨雾回升拆成足够完整的多条短字幕，不能为一 clip 一句牺牲画面变化。修订时应尽量保持审查表身份列 / 事实概要列稳定，只改写旁白字幕列，并在 audit 中记录样本来源、行数、生成 / 不生成数量、clip 标签覆盖、同 clip 拆分、合并跨度和禁止进入字幕的 GPS / 坐标 / mojibake / 泛类地理标签检查。
