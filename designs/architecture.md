...
1. `coarse-first analyze` 已把 ASR 纳入视频细扫前链路
   ...
   - `/chronology` 的 `span-rebuild` 对 speech/mixed 使用同 asset、同口播通道、间隔 `<=3000ms`、合并后 `<=45s` 且 transcript `<=160` 字的合并规则；visual 与 speech/mixed 可重叠共存，行车 visual 按 passage 降碎片；细粒度停顿继续由 `transcriptSegments` 表达
