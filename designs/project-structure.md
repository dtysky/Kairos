...
- Resolve 内直选字幕配音由独立插件目录 `apps/resolve-volc-voiceover-plugin/` 承载，安装到 DaVinci Resolve `Fusion/Scripts/Edit` 菜单；火山 API key / voice profiles 属于 workspace 全局 `config/runtime.json.voiceover`，生成音频属于项目级配音媒体根 `config/project-brief.json.voiceoverMedia`，通过 `/ingest-gps` 项目配置 UI 维护 primary / alternate path candidates，并由 `/edit` Resolve 重链动作处理 `Kairos Voiceover` Media Pool bin 的多设备 relink；正式输出目录为 `<voiceoverMediaRoot>/<safe Resolve project>/<safe timeline>/`，目录内只保存 `vo_<unit>_<request>.mp3`；请求缓存、插件 job / stdout / debug manifest / logs 属于项目 `.tmp/resolve-volc-voiceover-plugin/`，不写入项目正式 store，除非用户另行镜像审计产物
