...
class ImportedMediaSegment(ImportedSegment):
    ...
    def add_animation(self, animation_type, duration: Optional[Union[int, str]] = None) -> "ImportedMediaSegment":
        """为导入的视频片段添加动画

        Args:
            animation_type: 动画类型 (IntroType, OutroType, or GroupAnimationType)
            duration: 动画持续时间, 单位为微秒. 若传入字符串则会调用tim()函数进行解析.
                若不指定则使用动画类型定义的默认值.

        Returns:
            ImportedMediaSegment: 返回自身以支持链式调用
        """
        # 导入必要的类型（延迟导入避免循环依赖）
        from .metadata import IntroType, OutroType, GroupAnimationType
        from .animation import VideoAnimation
        from .video_segment import SegmentAnimations

        if duration is not None:
            duration = tim(duration)

        # 根据动画类型计算开始时间和持续时间
        if hasattr(animation_type, 'value'):  # IntroType, OutroType, GroupAnimationType
            if animation_type.__class__.__name__ == 'IntroType':
                start = 0
                duration = duration or animation_type.value.duration
            elif animation_type.__class__.__name__ == 'OutroType':
                duration = duration or animation_type.value.duration
                start = self.target_timerange.duration - duration
            elif animation_type.__class__.__name__ == 'GroupAnimationType':
                start = 0
        ...
        # 创建动画对象
        animation = VideoAnimation(animation_type, start, duration)

        # 在raw_data中添加动画信息
        if "extra_material_refs" not in self.raw_data:
            self.raw_data["extra_material_refs"] = []

        # 创建动画素材ID
        animation_id = uuid.uuid4().hex

        # 将动画ID添加到extra_material_refs中
        if animation_id not in self.raw_data["extra_material_refs"]:
            self.raw_data["extra_material_refs"].append(animation_id)

        # 在raw_data中存储动画信息（用于后续导出）
        if "animations" not in self.raw_data:
            self.raw_data["animations"] = []

        # 添加动画到列表中
        self.raw_data["animations"].append({
            "id": animation_id,
            "animation": animation.export_json()
        })
