...
def _build_material_id(path: str) -> str:
    """基于规范化后的素材路径生成稳定ID，避免不同目录同名文件冲突。"""
    normalized_path = os.path.normcase(os.path.realpath(os.path.abspath(path)))
    return uuid.uuid5(uuid.NAMESPACE_URL, normalized_path).hex
...
class VideoMaterial:
    ...
    def __init__(self, path: str, material_name: Optional[str] = None, crop_settings: CropSettings = CropSettings()):
        ...
        self.material_id = _build_material_id(path)
        self.path = path
