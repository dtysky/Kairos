...
- `导入与 GPS` 当前正式承载素材时间阻塞与修正：
  ...
  - `/chronology` 是 Analyze 和 Edit Flow 之间的正式审查页；先显式点击 `生成素材片段与模式` 运行 `span-rebuild`，由本地 qwen 文本 LM 按 candidate span 审查口播可用性并生成中文 `materialPatterns[]`；LM 返回 `keepSegmentIndexes / keepVisualOnly / materialPatterns` ordered rows，代码负责裁决 drop、visual-only、口播时间收缩和 slot 校验。Analyze 必须为 keep 的非音频素材产出可用视觉描述，`span-rebuild` 发现缺失 report 或缺失 `visualObservation` 会失败并要求回到 Analyze 修复；新 report 的 `fineScanWindows[]` 必须带稳定来源窗口血统，并由 speech/mixed 细扫窗口自己携带裁剪后的 transcript truth；旧 report 只在 fine-scan window 缺少 `semanticKind` 且自身或来源 `interestingWindow` 是 `speech-window` 并与 transcriptSegments 重叠时，才保守恢复 speech truth；明确的 `semanticKind=visual` 不因 transcript overlap 自动改成 speech；speech-backed candidate span 如果没有可用口播且没有独立视觉证据，不写入素材 span；全量收口后写入 stripped `store/spans.json` 与 `store/spans.meta.json`，再点击 `生成/刷新编年史` 运行 `chronology-build`
...
- official local runtime / monitor entry is `Supervisor + React console (apps/kairos-console/)`
  ...
  - `/chronology` 会展示活跃 `spatial-refresh / span-rebuild / chronology-build` jobs；`spatial-refresh / chronology-build` 不跑 ML，`span-rebuild` 会启动本地 ML 服务并按 8 个 span 一批调用 qwen 文本 LM 做口播可用性审查与 `materialPatterns` 归纳，LM 只返回 ordered review rows，不重跑 VLM / ASR，并把 chunk、失败列表补处理、retry/warning 进度写入 `projects/<projectId>/.tmp/chronology/progress.json`，已完成部分和 failed span 列表写入 `.tmp/chronology/span-rebuild.partial.json` 作为可恢复 checkpoint；正式 `store/spans.json` 仍只在全量收口后写入。`chronology-build` 也必须写入同一 progress 文件，并显示输入加载、Pharos/GPS、span 时空归属解析、row 排序、event/route 聚合、GPS 反查地名、gap 生成、review 状态合并和写入结果的计数进度，不能把旧 `span-rebuild` cache 当作当前 build 进度。
...
- Span rebuild now keeps span semantics intentionally small:
  - `store/spans.json` is only a material segment index derived from `store/assets.json` and `analysis/asset-reports/*.json`; `span-rebuild` uses the local qwen text LM through the ML service for speech usable-text review plus material pattern text generation
  ...
  - `materialPatterns` is a Chinese `string[]` derived only from the final reviewed span `type / semanticKind / transcript / transcriptSegments / visualObservation`, not labels, report summary, GPS, Pharos, time ownership, or span ids; the LM prompt requests `keepSegmentIndexes / keepVisualOnly / materialPatterns`, and code applies the decision before validating the 7 tags. Non-empty `keepSegmentIndexes` shrinks the span to usable transcript segments and forces slot 4 to `有口播语音`; empty speech plus `keepVisualOnly=true` is accepted only with independent `visualObservation`, clears speech truth, and forces slot 4 to `无口播语音`; empty speech without independent visual evidence drops the candidate span and ignores any returned patterns. Slot 1 must describe observable capture/composition evidence, not carrier type or edit purpose. Invalid/missing required slots enter the failed-span list, are retried once as a single span after the main chunks, and still block the rebuild if invalid after retry.
