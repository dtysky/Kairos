...
- rough-cut timeline placement now keeps effective source windows by default instead of fitting clips against `beat.targetDurationMs`; photos default to `1000ms` stills unless the edit rule / confirmed Flow Plan or runtime config explicitly declares another `timelineStillDurationMs`, and Resolve host validation blocks if the actual appended still duration differs
