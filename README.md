...
- the `/edit` console page is the formal Edit Flow surface after `/chronology`:
  ...
  - `timeline.generate` creates the Resolve rough-cut through the host by selecting already-synced Media Pool items in `material-slots` order, using chronology only for Resolve path/bin/context mapping, applying default source handles to selected audible speech/mixed non-photo clips (`240ms` head, `720ms` tail, clamped to asset bounds), validating source ranges after append, disabling linked audio items for muted video clips, marking Resolve clip colors for batching (`Orange` ordinary audible video/audio, `Blue` photo video items, `Purple` timelapse video items, with audible timelapse linked audio still `Orange`), creating/reusing Resolve Color Groups `Kairos Photos` and `Kairos Timelapse` for photo/timelapse video items, and writing `.tmp/edit-flow/<editId>/timeline/current.srt` from selected audible source-speech spans for manual DaVinci Resolve subtitle import; generated subtitle text must not auto-add or retain terminal `。` / `.` periods, while `？` / `！` remain valid; requested speed changes are currently ignored/pending in Resolve rough cuts
...
- subtitles now have a rough-cut companion path plus post-lock text paths:
  - `timeline.generate` writes `.tmp/edit-flow/<editId>/timeline/current.srt` from selected audible source-speech spans for manual Resolve import; generated SRT/VTT/review subtitle text strips terminal `。` / `.` periods and does not auto-supply them
