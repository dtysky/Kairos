...
- the `/edit` console page is the formal Edit Flow surface after `/chronology`:
  ...
  - current post-lock voiceover has an independent Resolve menu plugin at `apps/resolve-volc-voiceover-plugin/`: it runs inside DaVinci Resolve `Fusion/Scripts/Edit`, scans current subtitle tracks, lets the editor multi-select subtitle rows in the plugin list or select by playhead / Resolve In-Out, calls Volcengine TTS / voice clone APIs, and inserts one generated audio clip per subtitle into a `Kairos VO` audio track. This plugin is a direct Resolve tool, not a Supervisor job and not a required Flow Plan step.
...
- subtitles now have a rough-cut companion path plus post-lock text paths:
  ...
  - the Resolve Volc Voiceover plugin is the direct subtitle-to-voiceover path after subtitles already exist in the timeline. It reads subtitle items from Resolve, never guesses missing subtitle text, reads Volcengine API key and registered voice profiles from workspace global `config/runtime.json.voiceover`, resolves generated audio output from the Kairos project's `config/project-brief.json.voiceoverMedia` primary / alternate path candidates, and inserts generated clips without deleting existing user audio unless the editor explicitly chooses plugin-owned replacement.
