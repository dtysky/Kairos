...
- `导入与 GPS` 当前正式承载素材时间阻塞与修正：
  ...
  - `/analyze` 不生成 `store/spans.json` 或 `media/chronology.json`；它只产出 / 刷新 `analysis/asset-reports/*.json`，并可运行轻量 speech-boundary 补充分析，把每个素材的诊断明细写入 `analysis/speech-boundaries/<assetId>.json`
  - `/chronology` 是 Analyze 和 Edit Flow 之间的正式审查页；先显式点击 `生成候选素材片段与模式` 运行 `span-rebuild` span-builder，由本地 qwen 文本 LM 按 candidate span 生成 provisional 中文 `materialPatterns[]`，但不做最终 speech-window 裁切。Analyze 必须为 keep 的非音频素材产出可用视觉描述，`span-rebuild` 发现缺失 report 或缺失 `visualObservation` 会失败并要求回到 Analyze 修复；新 report 的 `fineScanWindows[]` 必须带稳定来源窗口血统，并由 speech/mixed 细扫窗口自己携带裁剪后的 transcript truth；旧 report 只在 fine-scan window 缺少 `semanticKind` 且自身或来源 `interestingWindow` 是 `speech-window` 并与 transcriptSegments 重叠时，才保守恢复 speech truth；明确的 `semanticKind=visual` 不因 transcript overlap 自动改成 speech；speech-boundary 补充分析只允许把 `effectiveSpeechStartMs / effectiveSpeechEndMs` 两个 source-ms 字段回填到 `store/spans.json` 的 speech/mixed span，作为后续原声 source window 的有效口播首尾，不覆盖 `sourceInMs/sourceOutMs` 或 transcriptSegments；如果存在 speech/mixed candidate span，span-builder 写入 `store/spans.json` 后必须把 `store/spans.meta.json.status` 标为 `pending-speech-review`，并写 `.tmp/chronology/speech-window-agent-handoff.md` 提醒用户去 Codex/Agent 启用 SubAgents 完成最终 speech-window 裁切；speech-window review 的每个 SubAgent shard 上限约 1500 candidates；只有 Agent 写回 `status=fresh` 后，才能点击 `生成/刷新编年史` 运行 `chronology-build`
  - 同一轮 Agent speech-window review 同时完成 ASR 后置字幕校对：`analysis/asset-reports/*.json` 中的 ASR 原文与 segment 时间保持不变，纠正只进入最终 speech/mixed `store/spans.json`，并写 `analysis/transcript-reviews/<inputsHash>.json` 审计。工作区共享词表位于 `config/transcript-glossary.json`，每项只保存唯一的“正确写法”、可选“发音”和必填“使用语境”；发音只帮助 Agent 召回可能的同音候选，不能触发机械替换，只有完整句子、相邻口播和当前时间段行程与该词条语境一致时才可采用。词表不得作为 ASR 热词、prompt 或 Qwen context 注入识别阶段，避免“瞬光 / 顺光”这类同音词被先验强推。无词表/行程依据的新专名必须进入 `/chronology` 的紧凑 Review Queue，全部确认并通过 hash/segment 原子校验前，`chronology-build` 持续被阻止
...
- official local runtime / monitor entry is `Supervisor + React console (apps/kairos-console/)`
  ...
  - `/chronology` 会展示活跃 `spatial-refresh / span-rebuild / chronology-build` jobs；`spatial-refresh / chronology-build` 不跑 ML，`span-rebuild` 会启动本地 ML 服务并按 8 个 span 一批调用 qwen 文本 LM 做 provisional `materialPatterns` 归纳，LM 只返回 ordered pattern rows，不重跑 VLM / ASR，也不做最终 speech-window 裁切，并把 chunk、失败列表补处理、retry/warning 进度写入 `projects/<projectId>/.tmp/chronology/progress.json`，已完成部分和 failed span 列表写入 `.tmp/chronology/span-rebuild.partial.json` 作为可恢复 checkpoint；`store/spans.json` 在 span-builder 收口后写入，但若含 speech/mixed candidate 则 meta 为 `pending-speech-review`，页面必须展示 Agent handoff。`chronology-build` 也必须写入同一 progress 文件，并显示输入加载、Pharos/GPS、span 时空归属解析、row 排序、event/route 聚合、GPS 反查地名、gap 生成、review 状态合并和写入结果的计数进度，不能把旧 `span-rebuild` cache 当作当前 build 进度。
...
- the `/edit` console page is the formal Edit Flow surface after `/chronology`:
  ...
  - `timeline.generate` creates the Resolve rough-cut through the host by selecting already-synced Media Pool items in `material-slots` order, using chronology only for Resolve path/bin/context mapping, applying default source handles to selected audible speech/mixed non-photo clips except aerial (`300ms` head, `300ms` tail, clamped to asset bounds), forcing aerial video linked audio disabled even if ASR produced speech truth, validating source ranges after append, disabling linked audio items for muted video clips, marking Resolve clip colors for batching (`Blue` photo video items, `Purple` timelapse video items, `Brown` drive video items, `Teal` aerial video items, `Orange` ordinary audible video items without a visual category color, and `Orange` for remaining audible linked audio items), creating/reusing Resolve Color Groups `Kairos Photos` and `Kairos Timelapse` for photo/timelapse video items, and writing `.tmp/edit-flow/<editId>/timeline/current.srt` from selected audible non-aerial source-speech spans for manual DaVinci Resolve subtitle import; generated subtitle text must not auto-add or retain terminal `。` / `.` periods, while `？` / `！` remain valid; requested speed changes are currently ignored/pending in Resolve rough cuts
...
- subtitles now have a rough-cut companion path plus post-lock text paths:
  - `timeline.generate` writes `.tmp/edit-flow/<editId>/timeline/current.srt` from selected audible non-aerial source-speech spans for manual Resolve import; aerial video is always muted and excluded from this source-speech SRT even when ASR hallucinated transcript truth; generated SRT/VTT/review subtitle text strips terminal `。` / `.` periods and does not auto-supply them
...
- `store/spans.json` is only a material segment index derived from `store/assets.json` and `analysis/asset-reports/*.json`; `span-rebuild` now acts as span-builder and uses the local qwen text LM through the ML service only for provisional material pattern text generation. Final speech-window cutting is a post span-builder Codex/Agent SubAgent step.
- Transcript correction is part of that same post span-builder review. It may replace segment text only, never segment timing or segmentation; corrected speech/mixed spans are downstream subtitle truth, while raw ASR reports remain immutable audit input.
...
- `store/spans.meta.json` records freshness, input hash, counts, warnings, and optional `speechReview` handoff metadata; `status=pending-speech-review` means candidate spans exist but Agent speech-window review must write final `fresh` spans before chronology-build. Hash excludes speed hints, Pharos, GPS cache, and chronology
...
- rough-cut timeline placement now keeps effective source windows by default instead of fitting clips against `beat.targetDurationMs`; selected audible speech/mixed non-photo clips receive default Resolve rough-cut source handles (`300ms` head and `300ms` tail, clamped to asset duration) so the generated cut does not land exactly on transcript boundaries, while source-speech SRT cues still map from the original transcript segment times inside those wider clip bounds; photos default to `1000ms` stills unless the edit rule / confirmed Flow Plan or runtime config explicitly declares another `timelineStillDurationMs`, and Resolve host validation blocks if the actual appended still duration differs
- `timeline.generate` may own local temporary audit artifacts before it creates/updates the Resolve rough cut:
  ...
  - `.tmp/edit-flow/<editId>/timeline/current.srt` stores the generated source-speech subtitle companion for manual Resolve import
