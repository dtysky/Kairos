...
- the `/edit` console page is the formal Edit Flow surface after `/chronology`:
  ...
  - `/edit` also exposes project-level Resolve `[Edit]` maintenance actions for the existing project: users may manually relink `Kairos Project Media` paths from the current `project-brief` root candidates to the readable local roots, and may save/register DRP snapshots. These maintenance buttons do not generate Flow Plans, run steps, or confirm gates.
