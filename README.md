...
- `导入与 GPS` 当前正式承载素材时间阻塞与修正：
  ...
  - 非 Pharos 单点的连续相邻静态素材按距离聚合：单 span 起止距离 `<=200m` 可视作静态，相邻代表点 `<=400m` 可合并，同地点只合并 chronology 顺序上的连续段；`drive` / route-like 车内素材，以及有跟车、跟随车辆、车辆行驶等明确移动主体证据的 `aerial` 航拍跟车素材，优先进入事件间 `route`，即使短 span 起止 GPS 距离很小也不得拆成普通 event；时间间隔本身不是切分阈值，行车口播 / 字幕只写入 route 摘要，不单独打断 route/event
...
- official local runtime / monitor entry is `Supervisor + React console (apps/kairos-console/)`
  ...
  - `/chronology` 会展示活跃 `spatial-refresh / span-rebuild / chronology-build` jobs；`spatial-refresh / chronology-build` 不跑 ML，`span-rebuild` 会启动本地 ML 服务并按 10 个 span 一批调用 qwen 文本 LM 做纯文本 materialPatterns 归纳，LM 只返回 ordered rows，不重跑 VLM / ASR，并把 chunk、失败列表补处理、retry/repair/warning/fallback 进度写入 `projects/<projectId>/.tmp/chronology/progress.json`，已完成部分和 failed span 列表写入 `.tmp/chronology/span-rebuild.partial.json` 作为可恢复 checkpoint；正式 `store/spans.json` 仍只在全量收口后写入。`chronology-build` 也必须写入同一 progress 文件，并显示输入加载、Pharos/GPS、span 时空归属解析、row 排序、event/route 聚合、GPS 反查地名、gap 生成、review 状态合并和写入结果的计数进度，不能把旧 `span-rebuild` cache 当作当前 build 进度。
