...
- independent `DaVinci color` chain now uses a minimal project-root + runtime/archive split:
  ...
  - current `/color` persists Resolve project sync snapshots under `color/resolve-projects/<safe-project-name>/`: root prepare exports one lightweight `.drp` only after all chunks finish and defaults to updating `latest.drp` only; manual DRP saving offers `覆盖最新` and `归档快照`, both update `latest.drp` and `color/resolve-project-map.json`, while only archive mode writes `snapshots/<timestamp>...drp`; externally exported `.drp` files can also be registered as latest archive entries
